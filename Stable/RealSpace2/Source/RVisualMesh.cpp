...
void AniFrameInfo::Frame(RAniMode amode,RVisualMesh* pVMesh)
{
	...
	if(m_bChangeAnimation) {
		...
		m_isPlayDone		= false;
		m_isOncePlayDone	= false;
		//애니메이션 이벤트를 다시 꺼주는 부분..
		if(m_pAniIdEventSet !=NULL)
			m_pAniNameEventSet = m_pAniIdEventSet->GetAniNameEventSet( m_pAniSet->GetName());	//일단 해당 애니메이션 셋을 찾고...
		if(m_pAniNameEventSet != NULL)
		{
			m_vecCheckAniEvent.clear();		//기존의 플래그는 날려버리고 
	...
	if( m_isPlayDone ) {// 1번은 play 한후..
		...
		if( m_pAniSet->GetAnimationLoopType() == RAniLoopType_Loop)
		{
			...
			if(m_pAniNameEventSet != NULL)
			{
				m_vecCheckAniEvent.clear();		//일단 기존에 쌓여있던 플래그들을 날려버리고 
				m_pAniNameEventSet->m_AniNameEventSetIter = m_pAniNameEventSet->m_AniNameEventSet.begin();
				while( m_pAniNameEventSet->m_AniNameEventSetIter != m_pAniNameEventSet->m_AniNameEventSet.end())
				{
					m_vecCheckAniEvent.push_back(false);
...
//	m_fSpeed = 2.1f;
	...
	m_1frame_time = cur;
	//애니메이션 이벤트 확인을 위한 부분..
	if(m_pAniNameEventSet != NULL)	//애니메이션 이벤트 정보가 있으면
	{
		m_pAniNameEventSet->m_AniNameEventSetIter = m_pAniNameEventSet->m_AniNameEventSet.begin();
...
//		무기,몬스터등 여러 VisualMesh 가있으니가 사용시점에서 생성..
		...
		if(!m_pTracks[mode]) {
			...
			m_pTracks[mode]->Create(7);//Trajectory length..
		}
	...
	if(m_pMesh) {
		...
		if(m_pMesh->GetPhysiqueMesh()) {
			...
			if(m_bIsRenderFirst) {
				...
				//엔진코드 보니 scale 성분이 worldmat와 따로 관리되고 있기 때문에 scale을 같이 넣어서 바운딩박스를 계산하는게 맞다.
				//그러나 수년간 그렇지 않은 코드로 서비스한지라 고치기 무섭다. 필요하다면 수동으로 m_bCalcBoxWithScale=true를 주어서 계산하자
...
//						(wtype != eq_wd_pistol) &&
						(wtype != eq_wd_shotgun) /*&&
						//잔탄 0일때 다른 무기로 자동 전환하는 기능이 상체 애니 완료 시점에 작동하는데
						//idle외에 아이템 샷 애니를 생략해버리면 자동 전환 기능이 작동하지 않게 되어서 아이템도 애니 섞기가 필요하다
						(wtype != eq_wd_item)
						*/ 
						// ↑ 유저들은 수리킷 의료킷을 벽을 보고 달리면서 번갈아 던져서 후다닥 먹는 방법을 쓰고 있었다
						// 달리면서 던지면 던지는 애니가 생략되어서 빨리 던질 수 있기 때문
						// 위에서 애니 생략을 하지않도록 했더니 수리킷 의료킷 던지는 애니메이션이 다시 생겨나서 유저들이
						// 딜레이가 느껴진다고 항의를 한다. (특히 캐쉬 의료킷을 산 유저들)
						// 다쓴무기 자동전환 안되는 버그를 수정하기보다는 기존 플레이감을 유지하는 편이 낫다고 판단하여 다시 주석처리함
						)
					{
						pAniUp->m_pAniSet = NULL;
						return true;
