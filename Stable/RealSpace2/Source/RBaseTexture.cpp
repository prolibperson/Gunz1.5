...
{
	const D3DPOOL pool = g_isDirect3D9ExEnabled ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED;
...
//		1/8 사이즈 : 4 <- HardwareTNL을 지원하지 않는 그래픽 카드일 경우
	...
	if( FAILED(D3DXCreateTextureFromFileInMemoryEx(
		...
		D3DX_FILTER_TRIANGLE | D3DX_FILTER_MIRROR,
		m_dwColorkey, &m_Info, NULL, &m_pTex ))) {
			_RPT1(_CRT_WARN,"%s ---->> memory texture (re)create failure \n",m_szTextureName);
...
RBaseTexture *RTextureManager::CreateBaseTexture(const char* filename,int tex_type,bool bUseMipmap,bool bUseFileSystem,DWORD colorkey)
{
	return CreateBaseTextureSub(false,filename, tex_type ,bUseMipmap, bUseFileSystem, colorkey);
}

RBaseTexture *RTextureManager::CreateBaseTextureMg(const char* filename,int tex_type,bool bUseMipmap,bool bUseFileSystem,DWORD colorkey)
{
	return CreateBaseTextureSub(true,filename, tex_type ,bUseMipmap, bUseFileSystem, colorkey);
}

RBaseTexture *RTextureManager::CreateBaseTextureSub(bool Mg,const char* filename,int tex_type,bool bUseMipmap,bool bUseFileSystem,DWORD colorkey)
{
...
RBaseTexture* RCreateBaseTexture(const char* filename,DWORD tex_type,bool bUseMipmap,bool bUseFileSystem,DWORD colorkey)
{
	return g_pTextureManager->CreateBaseTexture(filename,tex_type,bUseMipmap,bUseFileSystem,colorkey);
}

RBaseTexture* RCreateBaseTextureMg(const char* filename,DWORD tex_type,bool bUseMipmap,bool bUseFileSystem,DWORD colorkey)
{
	return g_pTextureManager->CreateBaseTextureMg(filename,tex_type,bUseMipmap,bUseFileSystem,colorkey);
}
