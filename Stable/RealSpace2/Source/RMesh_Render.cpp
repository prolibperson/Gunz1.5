...
{
	auto meshNode = std::find_if(m_list.begin(), m_list.end(), [&](RMeshNode* const node) {return node->m_PartsPosInfoType == type; });
	if (meshNode == m_list.end())
	{
		return nullptr;
	}
	return *meshNode;
...
void RMesh::RenderSub(D3DXMATRIX* world_mat,bool NoPartsChange,bool bRenderBuffer)
{
	...
	for (auto const& meshObj : m_list)
	{
		...
		pPartsMeshNode = meshObj;

		if (GetToolMesh()) {

			// 툴에서 특정 슬롯 부위만 렌더링하고자 할때
			if (m_OnlyRenderPartsType != eq_parts_end &&
				m_OnlyRenderPartsType != pPartsMeshNode->m_PartsType) {
				continue;
			}

			if (m_pToolSelectNode && meshObj == m_pToolSelectNode)// 툴이고 선택된 노드라면 박스그려주기...
				draw_box(&meshObj->m_mat_result, meshObj->m_max, meshObj->m_min, 0xffff0000);
		}

		if (m_pVisualMesh) {	// visual mesh 가 있는경우

			if (meshObj->m_PartsType == eq_parts_face && m_pVisualMesh->SkipRenderFaceParts())
			{
				continue;
			}

			if (NoPartsChange)	pPartsMeshNode = NULL;
			else				pPartsMeshNode = m_pVisualMesh->GetParts(meshObj->m_PartsType); // 장비된 모델 포인터 얻어내기..

			m_pVisualMesh->UpdateWeaponDummyMatrix(meshObj);

			if (meshObj->m_isWeaponMesh) { // 캐릭터에 달린 무기더미들 , 칼더미,총더미등..

				m_pVisualMesh->UpdateWeaponPartInfo(meshObj);
				...
				else if (pPartsMeshNode->m_PartsType != meshObj->m_PartsType)	// 같은 장착위치가 아니면
				{
					pPartsMeshNode = meshObj;
				}
			}
		...
		if (meshObj->m_isAddMeshNode) //pPartsMeshNode 조사하면 안됨.. pMeshNode 가 더미임...
		{
			if (meshObj->m_pParent) {
				pPartsMeshNode->m_mat_result = meshObj->m_mat_add * meshObj->m_pParent->m_mat_result;
			}
		}
		...
		if (pPartsMeshNode->m_isDummyMesh) { //Bip,Bone,Dummy Skip
			...
			if (m_pVisualMesh) {
				if (m_pVisualMesh->GetBipMatrixArray()) {
					if (meshObj->m_PartsPosInfoType != eq_parts_pos_info_etc) {
						m_pVisualMesh->GetBipMatrixArray()[meshObj->m_PartsPosInfoType] = meshObj->m_mat_result;
					}
				}
			}
			continue;
	...
	if( m_is_map_object && !mbSkyBox)
	{
		if(m_pVisualMesh)
		{
			if(m_pVisualMesh->GetTOCCL())
			{
				...
				if(m_pVisualMesh->IsCheckViewFrustum()) {//tool 이라면 false
...
void RMesh::RenderNode(RMeshNode *pMeshNode,D3DXMATRIX* world_mat)
{
	...
	if(m_pVisualMesh)
		if(m_pVisualMesh->IsRenderMatrix())//매트릭스 갱신만 원하는 경우라면....
			return;
...
bool RMesh::CalcIntersectsTriangle(rvector* vInVec, RPickInfo* pInfo, D3DXMATRIX* world_mat,bool fastmode)
{
	...
	for (auto const& mesh_Obj : m_list)
	{
		pPartsMeshNode = UpdateNodeAniMatrix(mesh_Obj);
