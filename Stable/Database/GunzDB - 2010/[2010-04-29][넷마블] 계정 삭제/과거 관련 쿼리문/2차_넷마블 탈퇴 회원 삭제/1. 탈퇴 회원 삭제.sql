...
(
	...
	CONSTRAINT NetmarbleBreakUser_PK PRIMARY KEY CLUSTERED (AID ASC)
)

CREATE TABLE dbo.NetmarbleBreakUserBackup
(
	UserCN 			VARCHAR(20)	NOT NULL,		
	UserID			VARCHAR(20)	NOT NULL,
	AID			INT		NOT NULL,
	...
	CONSTRAINT NetmarbleBreakUserBackup_PK PRIMARY KEY CLUSTERED (AID ASC)
)
...
AS BEGIN
	...
	---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	-- 백업을 위해서 모든 탈퇴한 회원을 저장해놓는다.
	INSERT NetmarbleBreakUserBackup(UserCN, UserID, AID, Break)
	SELECT UserCN, UserID, AID, BreakDate FROM dbo.NetmarbleBreakUser WHERE ErrCode IS NULL
