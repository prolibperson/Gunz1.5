...
BEGIN TRAN --------
	...
	INSERT Event_Alphabet_Temp(CID, ItemID, ItemCnt)
		...
		WHERE ci.ItemID BETWEEN 402000 AND 402025
...
BEGIN TRAN --------
	...
	CREATE TABLE Event_Alphabet_GameOn
	(
		CID INT,
		...
		CONSTRAINT Event_Alphabet_GameOn_PK PRIMARY KEY CLUSTERED (CID)
	)
	
	INSERT Event_Alphabet_GameOn(CID) SELECT CID FROM Event_Alphabet_Temp GROUP BY CID
	...
	WHILE( @ItemID != 402025 ) BEGIN
		...
		SET @strQuery = @strQuery + N' = t.ItemCnt FROM Event_Alphabet_GameOn e,  Event_Alphabet_Temp t WHERE e.CID = t.CID AND ItemID = ' + CONVERT(NVARCHAR, @ItemID);
