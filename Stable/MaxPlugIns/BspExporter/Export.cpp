...
void RbsExp::ExportLightObject(INode* node)
{
	...
	if(ls.type==OMNI_LIGHT)
	{
		...
		plight->Name= MSTR(node->GetName()).ToCStr();	
...
void RbsExp::ExportShapeObject(INode* node)
{
	...
	// 이름규칙
	WCHAR *part=StrStrW(node->GetName(),L"partition_");
	if(part)
	{
		int nPartitionNumber=_wtoi(part+wcslen(L"partition_"));
	...
	if(StrStrW(node->GetName(),L"wall_")!=NULL)
	{
		ShapeObject* shape = (ShapeObject*)os.obj;
		PolyShape pShape;
		int numLines;
		...
		poc->Name= MSTR(node->GetName()).ToCStr();	
...
void RbsExp::DumpMaterial(Mtl* mtl, int mtlID, int subNo)
{
	...
	TSTR className;
	mtl->GetClassName(className.ToMSTR());
	...
	if (mtl->ClassID() == Class_ID(DMTL_CLASS_ID, 0)) {
		...
		subm->Name= MSTR(mtl->GetName()).ToCStr();
		...
		if(_wcsnicmp(mtl->GetName(),L"at_",3)==0) subm->dwFlags|=RM_FLAG_USEALPHATEST;
	...
	if (mtl->NumSubMtls() > 0)  {
		...
		for (i=0; i<mtl->NumSubMtls(); i++) {
			...
			if (subMtl) {
				...
				(maxm->SubMaterials)[i]=rsm->MaxSubMaterialList.GetByName(subMtl->GetName().ToCStr());
			}
			else
			{
	...
	if(subm&&maxm) // top level & standard
	{
		...
		maxm->SubMaterials[0]=rsm->MaxSubMaterialList.GetByName(mtl->GetName().ToCStr());
	}
...
void RbsExp::DumpTexture(Texmap* tex, Class_ID cid, int subNo, float amt)
{
	...
	if (tex->ClassID() == Class_ID(BMTEX_CLASS_ID, 0x00)) {
		...
		switch(subNo) {
			case ID_DI:
			{
				TSTR mapName = MSTR(((BitmapTex *)tex)->GetMapName()).ToCStr();
...
void RbsExp::ExportHelperObject(INode* node)
{
	...
	// 더미만 익스포트한다.
	if (!_strnicmp(className.ToCStr(), "dummy", 5))
	{
		RDUMMY *pdm=new RDUMMY;
		pdm->name= MSTR(node->GetName()).ToCStr().dataForWrite();

		// 필요한 값이 있을때마다 추가해주도록 하자.
		int nPropValue=0;
		if (node->GetUserPropInt(L"time", nPropValue))
		{
			pdm->SetUserPropValue("time", nPropValue);
		}

		TSTR strValue;
		if(node->GetUserPropString(L"type", strValue.ToMSTR()))
		{
			pdm->SetUserPropValue("type", strValue.ToCStr().dataForWrite() );
		}
		if( node->GetUserPropString(L"min_point", strValue.ToMSTR()))
		{
			pdm->SetUserPropValue("min_point", strValue.ToCStr().dataForWrite() );
		}
		if( node->GetUserPropString(L"max_point", strValue.ToMSTR()))
		{
			pdm->SetUserPropValue("max_point", strValue.ToCStr().dataForWrite() );
		}
		if( node->GetUserPropString(L"r", strValue.ToMSTR()))
		{
			pdm->SetUserPropValue("r", strValue.ToCStr().dataForWrite() );
		}
		if( node->GetUserPropString(L"file", strValue.ToMSTR()))
		{
			pdm->SetUserPropValue("file", strValue.ToCStr().dataForWrite() );
		}
		...
		if ( _strnicmp( pdm->name.c_str(), cstrSmoke, sizeof(cstrSmoke) ) == 0 )
		{
//			pdm->SetUserPropValue(RTOK_SMOKE_NAME, filename );
			...
			if( node->GetUserPropFloat(L"size", fPropValue) ) 
				pdm->SetUserPropValue(RTOK_SMOKE_SIZE,fPropValue);

			if( node->GetUserPropString(L"color", cstrBuffer.ToMSTR()) ) 
				pdm->SetUserPropValue(RTOK_SMOKE_COLOR,cstrBuffer.ToCStr().dataForWrite());

			if( node->GetUserPropInt(L"delay", nPropValue) )
				pdm->SetUserPropValue(RTOK_SMOKE_DELAY,nPropValue);

			if( node->GetUserPropFloat(L"life",fPropValue) )
				pdm->SetUserPropValue(RTOK_SMOKE_LIFE,fPropValue);

			if( node->GetUserPropFloat(L"togglemintime",fPropValue) )
...
void RbsExp::ExportMesh(INode* node, TimeValue t)
{
	...
	log("object %s : ",MSTR(node->GetName()).ToCStr());
	...
	log("%s: %d ver / %d face \n",MSTR(node->GetName()).ToCStr(), poly->mm.VNum(),poly->mm.FNum());

	DWORD dwFlags=0;

	
	// 이름규칙들
	if(StrStrW(node->GetName(), L"nopath_")!=NULL)
		dwFlags|=RM_FLAG_NOTWALKABLE;
	if(StrStrW(node->GetName(),L"pass_")!=NULL)
		dwFlags|=RM_FLAG_PASSTHROUGH;
	if(StrStrW(node->GetName(),L"hide_")!=NULL)
		dwFlags|=RM_FLAG_HIDE;
	if(StrStrW(node->GetName(),L"passb_")!=NULL)
		dwFlags|=RM_FLAG_PASSBULLET;
	if(StrStrW(node->GetName(),L"passr_")!=NULL)
		dwFlags|=RM_FLAG_PASSROCKET;
	if(StrStrW(node->GetName(),L"hnav_")!=NULL)			// 퀘스트 AI용
	{
		dwFlags|=RM_FLAG_HIDE;
	...
	RCSGObject *pCObj=new RCSGObject;
	pCObj->m_Name=MSTR(node->GetName()).ToCStr();
	pCObj->m_pPolygons=new RSPolygonList;
