...
{
	// 맥스 2008 sdk 서 부터 wchar_t 를 안쓰고 mwchar_t ( = __wchar_t ) 를 쓰는데, 
	// visual studio 2005 서 부터는 wchar_t 를 내장형식으로 지정 옵션이 기본으로 켜져있어서 문제가 없는데
	// vs2003 은 기본이 꺼져있어서 이걸 켜면 다른 모든 라이브러리들이 바뀌어야 해서 얘만 강제 캐스팅했다.
#if defined(_3DSMAX2008) || defined(_3DSMAX2011)
	return TSTR( reinterpret_cast<__wchar_t*>( pwstr ) );
#else
	return MSTR(pwstr).ToCStr();
#endif
}
