...
//	memset(m_mtrl_data,0,MAX_NODE*4);
	...
	m_data.reserve(MAX_MTRL);//기본

	for(int i=0;i<MAX_MTRL;i++)
		m_data[i] = NULL;

	m_mtrl_data.reserve(MAX_MTRL);//기본
...
{
//  일정양 이상 예약하는것도 속도증가
//	if(m_data_num > MAX_MTRL-1) return -1;

	m_list.push_back(data);
	
	if( MAX_MTRL <= m_data_num)
	{
		// reserve()한 값보다 초과됐음
		char buff[256];
		sprintf(buff, "확보된 Mesh 메모리를 초과했습니다. 확인해주세요!");
...
//	if(m_mtrl_num > MAX_NODE-1) return -1;
	...
	if( MAX_MTRL <= m_mtrl_num)
	{
		// reserve()한 값보다 초과됐음
		char buff[256];
		sprintf(buff, "확보된 Material 메모리를 초과했습니다. 확인해주세요!");
		MessageBox( NULL, buff, "MCplug2 Error Message", MB_OK );
	}
