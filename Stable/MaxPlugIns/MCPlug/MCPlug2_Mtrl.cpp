...
void MCplug2::DumpMaterial(Mtl* mtl, int mtlID, int subNo)
{
	...
	if (mtl->ClassID() == Class_ID(DMTL_CLASS_ID, 0)) {
		...
		if(wcsnicmp(mtl->GetName(),L"at_",3)==0)//alpha_test 식별..
...
WCHAR* MCplug2::GetMapID(Class_ID cid, int subNo)
{
	static WCHAR buf[50];
	...
	if (cid == Class_ID(0,0)) {
		...
		switch (subNo) 	{
			...
			case ID_BU: wcscpy(buf, ID_MAP_BUMP);			break;
...
//		경로는 잘라버리고 이름만 저장
		...
		if(wcscmp(GetMapID(cid, subNo),ID_MAP_OPACITY)==0) {
			wcstombs(mtrl_node->m_opa_name,str,wcslen(str));
		}
	}
}
