				...
				if (vtxExport) {
					...
					if (vtxExport->GetVertexType() & BLENDED_TYPE) {
						...
						for (int n = 0; n < no; n++) {
							...
							wcstombs(mesh_node->m_physique[i].m_parent_name[n], Bone->GetName(), wcslen(Bone->GetName()));
						...
						wcstombs(mesh_node->m_physique[i].m_parent_name[0],Bone->GetName(),wcslen(Bone->GetName()));
...
void MCplug2::export_object_list(INode* node)
{
	...
	wcstombs(mesh_node->m_Name,FixupName(const_cast<WCHAR*>(node->GetName())),wcslen(node->GetName()));

	INode* parent = node->GetParentNode();

	if (parent && !parent->IsRootNode()) {

		wcstombs(mesh_node->m_Parent,FixupName(const_cast<WCHAR*>(parent->GetName())),wcslen(node->GetName()));
	}
