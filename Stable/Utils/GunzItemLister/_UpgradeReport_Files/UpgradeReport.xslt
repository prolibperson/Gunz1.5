...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
    ...
    <xsl:sort select="@Name" order="ascending"/>
        <h2>
        <xsl:if test="@solution">솔루션: <xsl:value-of select="@solution"/></xsl:if>
        <xsl:if test="not(@solution)">프로젝트: <xsl:value-of select="@name"/>
        ...
        <table cellpadding="2" cellspacing="0" width="98%" border="1" bordercolor="white" class="infotable">
            <tr>
                <td nowrap="1" class="header" _locID="Filename">파일 이름</td>
                <td nowrap="1" class="header" _locID="Status">상태</td>
                <td nowrap="1" class="header" _locID="Errors">오류</td>
                <td nowrap="1" class="header" _locID="Warnings">경고</td>
            </tr>
                ...
                <tr class="row">
                    ...
                    <td class="content">
                        <xsl:if test="count(event[@error-level='3'])=1">
                            ...
                            <xsl:if test="@description='Converted'">변환됨</xsl:if>
                            <xsl:if test="@description!='Converted'"><xsl:value-of select="@description"/></xsl:if>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="count(event[@error-level='3'])!=1 and count(event[@error-level='3' and @description='Converted'])!=0">변환됨
                        </xsl:if>
                    </td>
                ...
                <tr class="collapsed" bgcolor="#ffffff">
                    ...
                    <td colspan="7">
                        <table width="97%" border="1" bordercolor="#dcdcdc" rules="cols" class="issuetable">
                            <tr>
                                <td colspan="7" class="issuetitle" _locID="ConversionIssues">변환 문제점 - <xsl:value-of select="@name"/>:</td>
                            </tr>
            ...
            <tr valign="top">
                <td class="foot">
                    <xsl:if test="count(source)!=1">
                        <xsl:value-of select="count(source)"/> 파일
                    </xsl:if>
                    <xsl:if test="count(source)=1">
                        1 파일
                    </xsl:if>
                ...
                <td class="foot">
                    변환됨: <xsl:value-of select="count(source/event[@error-level='3' and @description='Converted'])"/><BR/>
                    변환되지 않음 <xsl:value-of select="count(source) - count(source/event[@error-level='3' and @description='Converted'])"/>
                </td>
    ...
    <xsl:template match="UpgradeLog">
        <html>
            ...
            <body topmargin="0" leftmargin="0" rightmargin="0" onclick="outliner();">
                <h1 _locID="ConversionReport">변환 보고서 - <xsl:value-of select="Properties/Property[@Name='Solution']/@Value"/></h1>

                <p><span class="note">
                <b>변환 시간:</b>  <xsl:value-of select="Properties/Property[@Name='Date']/@Value"/>  <xsl:value-of select="Properties/Property[@Name='Time']/@Value"/><br/>
                </span></p>
