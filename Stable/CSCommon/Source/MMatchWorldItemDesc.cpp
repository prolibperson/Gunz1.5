...
bool MMatchWorldItemDescMgr::ReadXml(const char* szFileName, MZFileSystem* pFileSystem)
{
...
void MMatchWorldItemDescMgr::ParseWorldItem(MXmlElement& element)
{
	...
	char szType[128];
	element.GetChildContents(szType, MWICTOK_TYPE);	if (!_stricmp(szType, "hp"))				pNewWorldItemDesc->m_nItemType = WIT_HP;
	...
	else if (!_stricmp(szType, "flag"))			pNewWorldItemDesc->m_nItemType = WIT_FLAG;
	else if (!_stricmp(szType, "spy_mine")) pNewWorldItemDesc->m_nItemType = WIT_MINE;
...
#define MMSITOK_DIRECTION	"DIRECTION"
