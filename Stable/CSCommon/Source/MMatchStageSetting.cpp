...
{
	Clear();
	//m_stageMods.reserve(STAGE_MOD::SM_END);
}
...
void MMatchStageSetting::SetDefault()
{
	...
	strcpy_s(m_StageSetting.Ref().szMapName, sizeof(m_StageSetting.Ref().szMapName),MMATCH_DEFAULT_STAGESETTING_MAPNAME);
...
{ 
	...
	if( strlen(pszName) < MAPNAME_LENGTH )
	{
		...
		strcpy_s(m_StageSetting.Ref().szMapName, sizeof(m_StageSetting.Ref().szMapName), pszName); // -- by SungE 2007-04-02
...
void MMatchStageSetting::SetMapIndex(int nMapIndex)
{
	...
	{
		strcpy_s(m_StageSetting.Ref().szMapName, sizeof(m_StageSetting.Ref().szMapName),MGetMapDescMgr()->GetMapName(nMapIndex)); 
	}
	m_StageSetting.MakeCrc();
}
...
//Custom: stagemod
bool MMatchStageSetting::isModEnabled(int mod)
{
	return std::find(m_stageMods.begin(), m_stageMods.end(), mod) != m_stageMods.end();
}

void MMatchStageSetting::SetStageMods(vector<int> mods)
{
