...
bool MMatchServer::StageAdd(MMatchChannel* pChannel, const char* pszStageName, bool bPrivate, const char* pszStagePassword, MUID* pAllocUID,bool bIsAllowNullChannel, vector<int> stageMods)
{
	...
	if (!pStage->Create( uidStage, pszStageName, bPrivate, pszStagePassword, bIsAllowNullChannel, stageMods,GameType, bIsCheckTicket, dwTicketID) ) {
		if (pChannel) {
...
//	if (pChannel->GetChannelType() == MCHANNEL_TYPE_LADDER) return false;
	...
	if (pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_SPYMODE || pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_INFECTION)
...
bool ExceptionTraceStageEnterBattle( MMatchObject* pObj, MMatchStage* pStage )
{
	...
	catch(...)//( StageEnterBattleExceptionHandler(GetExceptionInformation()) )
	{
...
bool MMatchServer::StageEnterBattle(const MUID& uidPlayer, const MUID& uidStage)
{
	...
	int ntype = -1;
	CopyCharInfoForTrans(&pNode->CharInfo, pObj->GetCharInfo(), pObj, ntype);
	...
	if (pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_INFECTION ||
		pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_SPYMODE)
	{
...
MCommand* MMatchServer::CreateCmdResponseStageSetting(const MUID& uidStage)
{
	...
	// Param 5 : stageMods
	void* stageListBlob = MMakeBlobArray(sizeof(int), pStage->GetStageSetting()->GetStageModCount());
	for (int i = 0; i < pStage->GetStageSetting()->GetStageModCount(); ++i) {
		*(int*)MGetBlobArrayElement(stageListBlob, i) = pStage->GetStageSetting()->GetStageMod(i);
	}
	pCmd->AddParameter(new MCmdParamBlob(stageListBlob, MGetBlobArraySize(stageListBlob)));
	MEraseBlobArray(stageListBlob);
...
void MMatchServer::OnStageCreate(const MUID& uidChar, char* pszStageName, bool bPrivate, char* pszStagePassword,vector<int> stageMods)
{
	...
	if (pChannel->GetRuleType() == MCHANNEL_RULE_EVENT && !IsAdminGrade(pObj))
	{
	...
	if (!StageAdd(pChannel, pszStageName, bPrivate, pszStagePassword, &uidStage,false,stageMods))
	{
		RouteResponseToListener(pObj, MC_MATCH_RESPONSE_STAGE_CREATE, MERR_CANNOT_CREATE_STAGE);
		return;
	}
...
void MMatchServer::OnPrivateStageJoin(const MUID& uidPlayer, const MUID& uidStage, char* pszPassword)
{
	...
	if (ugid == MMUG_DEVELOPER || ugid == MMUG_ADMIN || ugid == MMUG_STAFF || ugid == MMUG_EVENTMASTER)
		bSkipPassword = true;
...
void MMatchServer::OnStageRelayMapListUpdate(const MUID& uidStage, int nRelayMapType, int nRelayMapRepeatCount, void* pRelayMapListBlob)
{
	...
	// 블럭 만들기, 맵리스트 세팅
	void* relayMapListBlob = MMakeBlobArray(sizeof(MTD_RelayMap), pStage->GetRelayMapListCount());
	RelayMap RelayMapList[MAX_RELAYMAP_LIST_COUNT];
	...
	{
		MTD_RelayMap* pRelayMapList = (MTD_RelayMap*)MGetBlobArrayElement(relayMapListBlob, i);
...
//		pObj->SetInvalidStageSettingDisconnectWaitInfo();
	...
	if( MGetServerConfig()->IsEnabledSurvivalMode()==false && pNode->nGameType==MMATCH_GAMETYPE_SURVIVAL) {
		mlog(" stage setting game mode hack %s (%d, %d) ignore\n", pObj->GetName(), uidPlayer.High, uidPlayer.Low);
		char buff[164];
		sprintf(buff,"Stage Setting game mode Hack %s",pObj->GetName());
		MessageBox(NULL,buff,"Notice!",MB_OK);
		LogObjectCommandHistory( uidPlayer );
		pObj->DisconnectHacker( MMHT_INVALIDSTAGESETTING );
		return;
	}
	...
	if ((pChannel) && (bCheckChannelRule))
	{
		...
		if (pRule)
		{
			...
			{
				strcpy_s(pNode->szMapName, MGetMapDescMgr()->GetMapName(MMATCH_MAP_MANSION));
				pNode->nMapIndex = 0;
			}
			...
			{
				strcpy_s(pNode->szMapName, pSetting->GetMapName());
	...
	if (MGetGameTypeMgr()->IsQuestDerived(pNode->nGameType))
	{
		///TODO: write optional logic for this
		//if (pNode->bForcedEntryEnabled == true) pNode->bForcedEntryEnabled = false;
		///TODO: determine if this should be here anymore?
	...
	if (pNode->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE)
	{
		...
		if (pItem == nullptr) {
			mlog("error getting scenarionam %s\n",pScenario->GetDefaultScenarioName());
		}
		else
		{
	...
	//if ((MGetGameTypeMgr()->IsQuestDerived(nLastGameType) && (!MGetGameTypeMgr()->IsQuestDerived(pNode->nGameType))))
	...
	if (nLastGameType != pSetting->GetGameType())
	{
		...
		// 위의 모드들에서 기타 모드로 바뀐 경우 Mansion으로 세팅
...
void MMatchServer::OnGameReportTimeSync(const MUID& uidComm, unsigned long nLocalTimeStamp, unsigned int nDataChecksum)
{
	...
	if (fError > 2.f) {	
		...
		if (pSync->GetFoulCount() >= 3) {	// 3연속 스피드핵 검출 - 3진아웃
				...
				OnStageLeaveBattle(pObj->GetUID(), false);
				StageLeave(pObj->GetUID());//, pObj->GetStageUID());
...
//		}
	...
	if ((pOwnerChannel) && (!bSuicide))
	{
		if ((pOwnerChannel->GetRuleType() == MCHANNEL_RULE_MASTERY) || 
			...
			(pOwnerChannel->GetRuleType() == MCHANNEL_RULE_EVENT))
		{
			nVictimExp=0;
		}
	}
	...
	if ((pVictim->GetAccountInfo()->m_nUGrade == MMUG_ADMIN) || 
		...
		(pVictim->GetAccountInfo()->m_nUGrade == MMUG_STAFF) ||
		(pVictim->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER))
	{
	...
	if ((!bSuicide) &&
		...
		(pAttacker->GetAccountInfo()->m_nUGrade == MMUG_STAFF)||
		(pAttacker->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER)))
	{
		nVictimExp = 0;
	}
	...
	if(pPlayer->GetCharInfo()->m_nBattleStartTime != 0)
	{
		...
		{// 이상적으로 값이 세팅돼면 로그를 남긴다.
			...
			if( pStage != NULL )
			{
				...
				case MMATCH_GAMETYPE_QUEST_CHALLENGE:	{sprintf(buf, "QUEST_CHALLENGE");	} break;		///< 챌린지 퀘스트
...
void MMatchServer::StageList(const MUID& uidPlayer, int nStageStartIndex, bool bCacheUpdate)
{
	...
	for (int i = /*nStageStartIndex*/nRealStageStartIndex; i < pChannel->GetMaxPlayers(); i++)
	{
		...
		strcpy_s(pNode->szStageName, pStage->GetName());
		...
		for (int i = 0; i < STAGE_MOD::SM_END; ++i)
			pNode->stageMods[i] = 0;

		for (int i = 0; i < pStage->GetStageSetting()->GetStageModCount(); ++i)
		{
			pNode->stageMods[i] = pStage->GetStageSetting()->GetStageMods().at(i);
		}
...
//Custom: Demolition
void MMatchServer::OnRequestPlantBomb(const MUID& planter)
{
	MMatchObject* pObj = GetObject(planter);
	if (!IsEnabledObject(pObj)) return;
	MMatchStage* pStage = FindStage(pObj->GetStageUID());
	if (pStage == NULL) return;
	...
	if (pStage->GetStageSetting())
	{
		if (pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_DEMOLITION)
		{
			MMatchRuleDemolition* pRule = (MMatchRuleDemolition*)pStage->GetRule();
			if (pRule)
			{
				pRule->OnPlantBomb(planter);
			}
		}
	}
}

//Custom: checkstagemods
void MMatchServer::OnCheckStageModHacks(vector<int> stageMods, const MUID& player)
{
	MMatchObject* pObj = GetObject(player);
	if (!IsEnabledObject(pObj))
		return;
	...
	if (stageMods.size() != pStage->GetStageSetting()->GetStageMods().size())
		OnAdminBanPlayer(pObj, MMHID_STAGEMODHACK);


	for (size_t i = 0; i < stageMods.size(); ++i)
	{
		if(!pStage->GetStageSetting()->isModEnabled(stageMods.at(i)))
			OnAdminBanPlayer(pObj,MMHID_STAGEMODHACK);
	}
}
