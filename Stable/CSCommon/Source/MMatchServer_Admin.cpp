...
//		DisconnectObject(uidAdmin);		
	...
	if (m_Admin.Execute(uidAdmin, szText))
	{
		...
		pNew->AddParameter(new MCmdParamStr(szText));
...
void MMatchServer::OnResponseAdminSlap(const MUID& admin, const MUID& target)
{
	...
	if (!IsAdminGrade(adminObj))
	{
		OnAdminBanPlayer(adminObj, MMatchHackID::MMHID_ADMINHACK);
		return;
	}
...
//Custom: ban player
void MMatchServer::OnAdminBanPlayer(MMatchObject* pObj, MMatchHackID hackID)
{
	char bannedReason[20];

	switch (hackID)
	{
		case MMHID_STAGEMODHACK:
		{
			strcpy_s(bannedReason, sizeof(bannedReason), "StageMod Hacking");
		}break;
		case MMHID_ADMINHACK:
		{
			strcpy_s(bannedReason, sizeof(bannedReason), "Admin Hack");
		}break;
		default:
		{
			strcpy_s(bannedReason, sizeof(bannedReason), "Unknown");
		}break;
	}

	GetDBMgr()->OnBanPlayer(pObj->GetAccountInfo()->m_nAID, hackID,bannedReason);
