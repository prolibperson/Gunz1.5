...
TCHAR g_szSetHackingBlock[] = _T( "{CALL  LogDB15..spSetHackingBlock (%u, %u, %u, %u, %u, %u, %u, '%s', %u, '%s', '%s', %u)}" );
...
//TODO: Add addedkillcount/deathcount?
//TCHAR g_szDB_UPDATE_CHAR_INFO_DATA[] = _T("{CALL spUpdateCharInfoData (%d, %d, %d, %d, %d, %d, %d, %d)}");
...
//TODO: recreate missing stored procedure
...
bool MMatchDBMgr::GetAccountCharList(const int nAID, MTD_AccountCharInfo* poutCharList, int* noutCharCount)
{
	...
	for( ; ! rs.IsEOF(); rs.MoveNext() )
	{
		...
		strcpy_s(poutCharList[t].szName, (LPCTSTR)rs.Field("Name").AsString());
...
bool MMatchDBMgr::GetAccountCharInfo(const int nAID, const int nCharIndex, MTD_CharInfo* poutCharInfo)
{
	...
	strcpy_s(poutCharInfo->szName, (LPCTSTR)rs.Field("Name").AsString());
	strcpy_s(poutCharInfo->szClanName, (LPCTSTR)rs.Field("ClanName").AsString());
...
bool MMatchDBMgr::GetCharInfoByAID( const int nAID, const int nCharIndex, MMatchCharInfo* poutCharInfo )
{
	...
	strcpy_s(poutCharInfo->m_szName, (LPCTSTR)db_strName);
	...
	strcpy_s(poutCharInfo->m_ClanInfo.m_szClanName, (LPCTSTR)db_strClanName);
...
bool MMatchDBMgr::GetAccountInfo( const unsigned long int nAID, MMatchAccountInfo* poutAccountInfo, const int nServerID )
{
	...
	strcpy_s(poutAccountInfo->m_szUserID, (LPCTSTR)rs.Field("UserID").AsString());
	...
	else 
	{
		...
		if( MMHID_STAGEMODHACK == poutAccountInfo->m_HackingType != poutAccountInfo->m_HackingType || MMHT_SLEEP_ACCOUNT != poutAccountInfo->m_HackingType )
		{
...
bool MMatchDBMgr::GetCharItemInfo(MMatchCharInfo* pCharInfo)
{
	...
	for( ; ! rs.IsEOF(); rs.MoveNext() ) 
	{
		...
		if( nCnt <= 0 ) continue;										///< He has no item of this kind.
		if(  300010 < nItemDescID && 300040 > nItemDescID) continue;	///< AI NPC Weapons	

		if( rs.Field("RentHourPeriod").IsNull() ) 	
			wMaxUsableHour = RENT_PERIOD_UNLIMITED;	
		else 
			wMaxUsableHour = (WORD)rs.Field("RentHourPeriod").AsShort();
		...
		MMatchItemDesc *pItemDesc = MGetMatchItemDescMgr()->GetItemDesc(nItemDescID);
		if (pItemDesc != NULL )	//< Normal Item
		{							
												...
												const int nBounty, const int nCharBP, const _ITEMPURCHASE_TYPE nType)
{
	...
	switch (nType)
	{
	case IPT_BUY: strcpy_s(szType, "구입"); break;
	case IPT_SELL: strcpy_s(szType, "판매"); break;
	}
									  ...
									  const _CHARMAKING_TYPE nType)
{
	...
	switch (nType)
	{
	case CMT_CREATE: strcpy_s(szType, "생성"); break;
	case CMT_DELETE: strcpy_s(szType, "삭제"); break;
	}
...
bool MMatchDBMgr::UpdateCharInfoData(const int nCID, const int nAddedXP, const int nAddedBP,
	const int nAddedKillCount, const int nAddedDeathCount, const int nAddedPlayTime, const int nAddedRankedKillCount, const int nAddedRankedDeathCount)
{
	//TODO: add addedrankedkillcount/deathcount????
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_UPDATE_CHAR_INFO_DATA, nCID, nAddedXP, nAddedBP,
			nAddedKillCount, nAddedDeathCount, nAddedPlayTime/*, nAddedRankedKillCount, nAddedRankedDeathCount*/);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e) {
...
bool MMatchDBMgr::GetLadderTeamID(const int nTeamTableIndex, const int* pnMemberCIDArray, const int nMemberCount, int* pnoutTID)
{
	...
	//#ifdef _DEBUG
	strcpy_s(szFieldName, "TID4");	// 지금은 모두다 TID4로 통일중
	//#else
...
bool MMatchDBMgr::GetClanInfo(const int nCLID, MDB_ClanInfo* poutClanInfo)
{
	...
	strcpy_s(poutClanInfo->szClanName , rs.Field("Name").AsString());
	strcpy_s(poutClanInfo->szMasterName , rs.Field("ClanMaster").AsString());
	strcpy_s(poutClanInfo->szEmblemUrl , rs.Field("EmblemUrl").AsString());
...
bool MMatchDBMgr::InsertQuestGameLog( const char* pszStageName, 
									 ...
									 const int nMasterCID, const int nPlayer1, const int nPlayer2, const int nPlayer3, const int nTotalRewardQItemCount, const int nElapsedPlayTime, int& outQGLID )
{
	_STATUS_DB_START;
	if( !CheckOpen() )
		return false;
...
bool MMatchDBMgr::GetSurvivalModeGroupRanking(vector<RANKINGINFO*> pRankingVec[])
{
	...
	for( int nIndex = 0; ! rs.IsEOF(); rs.MoveNext() )
	{
		...
		strcpy_s(pRankingInfo->szCharName, rs.Field("Name").AsString());
...
bool MMatchDBMgr::GetDuelTournamentTimeStamp(char *szTimeStamp)
{
	...
	memcpy(szTimeStamp, rs.Field("tStamp").AsString(), DUELTOURNAMENT_TIMESTAMP_MAX_LENGTH + 1);
...
bool MMatchDBMgr::GetDuelTournamentSideRankingInfo(DWORD dwCID, list<DTRankingInfo*> *pRankingList)
{
	...
	for( int nIndex = 0; ! rs.IsEOF(); rs.MoveNext() )
	{
		...
		strcpy_s(pInfo->m_szCharName, rs.Field("Name").AsString());
											  ...
											  int nPlayer5CID, int nPlayer6CID, int nPlayer7CID, int nPlayer8CID, int *nOutNumber, char *szOutTimeStamp)
{
	...
	*nOutNumber = rs.Field("LogID").AsInt();
	strcpy(szOutTimeStamp, rs.Field("tStamp").AsString());
...
bool MMatchDBMgr::GetBattletimeRewardList(vector<MMatchBRDescription*>& vBattletimeRewardDescription)
{
	...
	for( ;! rs.IsEOF(); rs.MoveNext() )
	{
		...
		int nBRTID = 1; //rs.Field("BRTID").AsInt(); TODO:repair
...
//Custom: ban player
bool MMatchDBMgr::OnBanPlayer(const int AID, const MMatchHackID hackID,const char* banReason)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_ON_BAN_PLAYER, AID,hackID,banReason);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e) {
		ExceptionHandler(strSQL, e);
		Log("MMatchDBMgr::OnBanPlayer failed\n");
		return false;
	}
...
bool MMatchDBMgr::UpdateSkillMapBestTIme(const unsigned int player, const int& mapID, const unsigned int bestTime)
{
	...
	try {
		strSQL.Format(g_szDB_UPDATE_SKILLMAP_BESTTIME, player, mapID, bestTime);// , banReason);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e) {
		ExceptionHandler(strSQL, e);
