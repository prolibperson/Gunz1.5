...
bool MMatchStage::Create(const MUID& uid, const char* pszName, bool bPrivate, const char* pszPassword, bool bIsAllowNullChannel, vector<int> stageMods,
	const MMATCH_GAMETYPE GameType, const bool bIsCheckTicket, const DWORD dwTicketItemID)
{
	...
	m_nStageType = MST_NORMAL;
	m_uidStage = uid;
	strcpy_s(m_szStageName,sizeof(m_szStageName), pszName);
	strcpy_s(m_szStagePassword, sizeof(m_szStagePassword),pszPassword);
	m_bPrivate = bPrivate;
	...
	m_StageSetting.SetStageMods(stageMods);
...
void MMatchStage::EnterBattle(MMatchObject* pObj)
{
	...
	if (GetState() == STAGE_STATE_RUN)
	{
		...
		{
			if (m_StageSetting.IsWaitforRoundEnd() || m_StageSetting.GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE || m_StageSetting.IsQuestDrived())
			{
				pObj->SetAlive(false);
			}
...
/*	case MMATCH_GAMETYPE_CLASSIC_SOLO:
	...
	{
		return (new MMatchRuleInfection(this));
	}break;
	...
	{
		return (new MMatchRuleBlitzKrieg(this));
	}break;
	...
	{
		return (new MMatchRuleDemolition(this));
	}break;
...
bool MMatchStage::CheckBattleEntry()
{
	...
	for (MUIDRefCache::iterator i=m_ObjUIDCaches.begin(); i!=m_ObjUIDCaches.end(); i++) {
		MMatchObject* pObj = (MMatchObject*)(*i).second;
		if (pObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) == true && IsAdminGrade(pObj))
			continue;
...
int MMatchStage::GetObjInBattleCount()
{
	...
	for (MUIDRefCache::iterator itor=GetObjBegin(); itor!=GetObjEnd(); ++itor) 
	{
		...
		if (pObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) == true || pObj->GetPlace() != MMP_BATTLE)
			continue;
...
void MMatchStage::OnGameKill(const MUID& uidAttacker, const MUID& uidVictim)
{
	...
	MMatchObject* player = MGetMatchServer()->GetObject(uidAttacker);
...
void MMatchStage::ShuffleTeamMembers()
{
	...
	if ((m_nStageType == MST_LADDER) || (m_StageSetting.IsTeamPlay() == false) ||
		(m_StageSetting.GetGameType() == MMATCH_GAMETYPE_INFECTION)||
...
bool MMatchStage::IsValidMap( const char* pMapName )
{
	...
	// 챌린지퀘스트 맵 검사는 별도로 처리 //todok 일단 패스시켰지만 출시전엔 올바른 맵이름 검증을 구현해야 한다
