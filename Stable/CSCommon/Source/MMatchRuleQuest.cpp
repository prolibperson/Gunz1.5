...
//	MQuestPlayerInfo* pPlayerInfo = m_PlayerManager.GetPlayerInfo(uidPlayer);
	...
	//i hate my life and i wanna die i aint got no huawei
	unsigned long nNowTime = MMatchServer::GetInstance()->GetTickTime();
	if ((nNowTime - m_nPrepareStartTime) >= (PORTAL_MOVING_TIME - 1000))    
		return;            
...
bool MMatchRuleQuest::PrepareStart()
{
	if( MSM_CLAN == MGetServerConfig()->GetServerMode() ) 
	{
		...
		{
			//mlog("Something's wrong with the ql gameinfo, ql game info value %d", m_StageGameInfo.nQL);
#ifdef _DEBUG
#else
