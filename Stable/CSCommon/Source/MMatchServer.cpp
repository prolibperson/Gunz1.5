...
constexpr auto DEFAULT_REQUEST_UID_SIZE = 4200000000;///< UID 할당 요청 기본 단위
constexpr auto DEFAULT_REQUEST_UID_SPARE_SIZE = 10000;	///< UID 남은 갯수
constexpr auto DEFAULT_ASYNCPROXY_THREADPOOL = 6;
constexpr auto MAXUSER_WEIGHT = 30;

constexpr auto MAX_DB_QUERY_COUNT_OUT = 5;	// 쿼리가 5번이상 실패하면 Shutdown


constexpr auto MATCHSERVER_DEFAULT_UDP_PORT = 7777;

constexpr auto FILENAME_ITEM_DESC = "zitem.xml";
constexpr auto FILENAME_ITEM_DESC_LOCALE = "zitem_locale.xml";
constexpr auto FILENAME_BUFF_DESC = "zBuff.xml";
constexpr auto FILENAME_SHOP = "shop.xml";
constexpr auto FILENAME_CHANNEL = "channel.xml";
constexpr auto FILENAME_SHUTDOWN_NOTIFY = "shutdown.xml";
constexpr auto FILENAME_WORLDITEM_DESC = "worlditem.xml";
constexpr auto FILENAME_MONSTERGROUP_DESC = "monstergroup.xml";
constexpr auto FILENAME_CHANNELRULE = "channelrule.xml";
...
void CopyCharInfoForTrans(MTD_CharInfo* pDest, MMatchCharInfo* pSrcCharInfo, MMatchObject* pSrcObject,int nSetEquip)
{
	memset(pDest, 0, sizeof(MTD_CharInfo));

	if (pSrcCharInfo)
	{
		strcpy_s(pDest->szName, pSrcCharInfo->m_szName);
		strcpy_s(pDest->szClanName, pSrcCharInfo->m_ClanInfo.m_szClanName);
		...
		{
			pSrcCharInfo->m_nTEquipedItemID[MMCIP_MELEE] = 1;
			pSrcCharInfo->m_nTEquipedItemID[MMCIP_PRIMARY] = 4001;
			pSrcCharInfo->m_nTEquipedItemID[MMCIP_SECONDARY] = 0;
			pSrcCharInfo->m_nTEquipedItemID[MMCIP_FINGERL] = 0;
			pSrcCharInfo->m_nTEquipedItemID[MMCIP_FINGERR] = 0;
			pSrcCharInfo->m_nTEquipedItemID[MMCIP_CUSTOM1] = 0;
			pSrcCharInfo->m_nTEquipedItemID[MMCIP_CUSTOM2] = 0;
			if (pSrcCharInfo->m_nSex == 1)
			{
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_HEAD] = RandomNumber(520501, 520521);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_CHEST] = RandomNumber(521501, 521521);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_HANDS] = RandomNumber(522501, 522521);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_LEGS] = RandomNumber(523501, 523521);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_FEET] = RandomNumber(524501, 524521);
			}
			else
			{
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_HEAD] = RandomNumber(520001, 520021);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_CHEST] = RandomNumber(521001, 521021);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_HANDS] = RandomNumber(522001, 522021);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_LEGS] = RandomNumber(523001, 523021);
				pSrcCharInfo->m_nTEquipedItemID[MMCIP_FEET] = RandomNumber(524001, 524021);
			}
			for (int i = 0; i < MMCIP_END; i++)
			{
				pDest->nEquipedItemDesc[i] = pSrcCharInfo->m_nTEquipedItemID[i];
				if (pSrcCharInfo->m_nTEquipedItemID[i] == 0)
					pDest->nEquipedItemCount[i] = 0;
				else
					pDest->nEquipedItemCount[i] = 1;
			}
		}
		else if (nSetEquip == 2)
		{
...
*/
	...
	/*if (!m_SecurityMgr.Create("SecurityManager.xml"))
	{
		Log(LOG_FILE, "Read SecuritySettings has Failed\n");
		return false;
	}*/
	...
	// 기존 퀘스트 이후에 해야 한다 (드롭테이블 먼저 읽어야 하므로)
	IMatchRuleNewQuest::InitActorDefMgr();
	...
	if (!MGetSpyMode()->ReadMapList("spymaplist.xml"))
	{
		Log(LOG_PROG, "Read spymaplist.xml failed\n");
	}
	if (!MGetSpyMode()->ReadMode("spymode.xml"))
	{
		Log(LOG_PROG, "Read spymode.xml failed\n");
	}
	
	if (!MGetBlitz()->ReadXml("blitzkrieg.xml"))
	{
		Log(LOG_PROG, "read blitzkrieg.xml failed\n");
	}

	if (!MGetAchievements()->readXml("achievements.xml"))
		Log(LOG_PROG, "read achievements.xml failed\n");
...
{
	constexpr auto MTOK_DEFAULTCHANNELNAME = "DEFAULTCHANNELNAME";
	constexpr auto MTOK_DEFAULTRULENAME = "DEFAULTRULENAME";
	constexpr auto MTOK_CHANNEL = "CHANNEL";
	constexpr auto MTOK_CLAN_CHANNEL = "CLAN_CHANNEL";

	MZFile mzf;
	if (!mzf.Open(FILENAME_CHANNEL)) {
		mlog("Error opening %s", FILENAME_CHANNEL);
		return false;
	}
	char* buffer = new char[mzf.GetLength() + 1];
	buffer[mzf.GetLength()] = 0;
	if (!mzf.Read(buffer, mzf.GetLength())) {
		mlog("Error reading %s", FILENAME_CHANNEL);
		delete[] buffer;
		return false;
	}
	rapidxml::xml_document<> doc;
	if (!doc.parse<0>(buffer)) {
		mlog("error parsing %s", FILENAME_CHANNEL);
		delete[] buffer;
		return false;
	}
	...
	for (auto itor = rootNode->first_node(); itor; itor = itor->next_sibling()) {
		...
		if (_stricmp(itor->name(), MTOK_CHANNEL) == 0) {
			...
			if (itor->first_attribute("rule") != nullptr)
				strcpy_s(szRuleName, itor->first_attribute("rule")->value());
			else
				strcpy_s(szRuleName, GetDefaultChannelRuleName());
			if (itor->first_attribute("IsDuelTournament") != nullptr)
			{
				if (_stricmp(itor->first_attribute("IsDuelTournament")->value(), "true") == 0)
					nChannelType = MCHANNEL_TYPE_DUELTOURNAMENT;
			}
			if (itor->first_attribute("IsTicketChannel") != nullptr)
			{
				if (_stricmp(itor->first_attribute("IsTicketChannel")->value(), "true"))
					bIsTicketChannel = true;
			}
			if (itor->first_attribute("maxplayers") != nullptr)
				nMaxPlayers = atoi(itor->first_attribute("maxplayers")->value());
			if (itor->first_attribute("levelmin") != nullptr)
				nLevelMin = atoi(itor->first_attribute("levelmin")->value());
			if (itor->first_attribute("levelmax") != nullptr)
				nLevelMax = atoi(itor->first_attribute("levelmax")->value());
			if (itor->first_attribute("TicketItemID") != nullptr)
				nTicketItemID = atoi(itor->first_attribute("TicketItemID")->value());
			MUID uidChannel;
			ChannelAdd(MGetStringResManager()->GetStringFromXml(itor->first_attribute("name")->value()), szRuleName, &uidChannel, nChannelType,
				nMaxPlayers, nLevelMin, nLevelMax, bIsTicketChannel, nTicketItemID, bIsTicketChannel, itor->first_attribute("name")->value());

		}
		if (_stricmp(itor->name(), MTOK_DEFAULTRULENAME) == 0)
			SetDefaultChannelName(MGetStringResManager()->GetStringFromXml(itor->first_attribute("name")->value()));
		if (_stricmp(itor->name(), MTOK_DEFAULTRULENAME) == 0)
			SetDefaultChannelRuleName(itor->first_attribute("name")->value());
		if (_stricmp(itor->name(), MTOK_CLAN_CHANNEL) == 0)
		{
			bool	bIsTicketChannel = false;
			int		nTicketItemID = -1;
			...
			if (itor->first_attribute("IsTicketChannel") != nullptr)
			{
				if (_stricmp(itor->first_attribute("IsTicketChannel")->value(), "true") == 0)
					bIsTicketChannel = true;
			}
			if (itor->first_attribute("TicketItemID") != nullptr)
			{
...
//#endif
	...
	///TODO: uncomment after i replace this missing table
...
// #endif
	...
	constexpr auto MINTERVAL_GARBAGE_SESSION_PING = (3 * 60 * 1000);	// 3 min
...
//	CheckMemoryCorruption();
	...
	constexpr auto MINTERVAL_GARBAGE_SESSION_CLEANING = 10 * 60 * 1000;	// 10 min
...
void MMatchServer::UpdateServerLog()
{
	...
	constexpr auto SERVER_LOG_TICK = (60000);	// 1분 (1000 * 60)
...
void MMatchServer::UpdateServerStatusDB()
{
	...
	constexpr auto SERVER_STATUS_TICK(30000);	// 30초 (1000 * 30)
	...
	case MSGID_RAWCOMMAND:
		{
			...
			if (pPacketHeader->nCheckSum != nCheckSum) {
				...
				{
					for (auto itor = MGetMatchServer()->GetObjects()->begin(); itor != MGetMatchServer()->GetObjects()->end(); ++itor)
					{
						if ((*itor).second->GetIP() == dwIP)
						{
							(*itor).second->DisconnectHacker(MMHT_COMMAND_FLOODING);
							mlog("Disconnect flooder %s", (*itor).second->GetCharInfo()->m_szName);
							return;
						}
					}
				}
...
void MMatchServer::ResponsePeerList(const MUID& uidChar, const MUID& uidStage)
{
	...
	for (MUIDRefCache::iterator itor=pStage->GetObjBegin(); itor!=pStage->GetObjEnd(); itor++)
	{
		...
		if (player->GetEnterBattle() == false) continue;
		if (!IsEnabledObject(player)) continue;

		MTD_PeerListNode* pNode = (MTD_PeerListNode*)MGetBlobArrayElement(pPeerArray, nIndex++);
		memset(pNode, 0, sizeof(MTD_PeerListNode));
		
		pNode->uidChar = player->GetUID();
		pNode->dwIP = player->GetIP();
		pNode->nPort = player->GetPort();
		int ntype = -1;

		CopyCharInfoForTrans(&pNode->CharInfo, player->GetCharInfo(), player,ntype);
		//버프정보임시주석 CopyCharBuffInfoForTrans(&pNode->CharBuffInfo, pObj->GetCharInfo(), pObj);

		pNode->ExtendInfo.nPlayerFlags = player->GetPlayerFlags();
		if (pStage->GetStageSetting()->IsTeamPlay())	pNode->ExtendInfo.nTeam = (char)player->GetTeam();
...
void MMatchServer::UpdateCharDBCachingData(MMatchObject* pObject)
{
	...
	int	nAddedXP, nAddedBP, nAddedKillCount, nAddedDeathCount, nAddedPlayTime, nAddedRankedKillCount, nAddedRankedDeathCount;
	...
	nAddedRankedKillCount = 0;// pObject->GetCharInfo()->GetDBCachingData()->nAddedRankedKillCount;
	nAddedRankedDeathCount = 0;// pObject->GetCharInfo()->GetDBCachingData()->nAddedRankedDeathCount;

	if ((nAddedXP != 0) || (nAddedBP != 0) || (nAddedKillCount != 0) || (nAddedDeathCount != 0) || (nAddedPlayTime >= 0) || (nAddedRankedKillCount != 0) || (nAddedRankedDeathCount != 0))
	{
		MAsyncDBJob_UpdateCharInfoData* pJob = new MAsyncDBJob_UpdateCharInfoData(pObject->GetUID());
		pJob->Input(pObject->GetCharInfo()->m_nCID, nAddedXP, nAddedBP, nAddedKillCount, nAddedDeathCount, nAddedPlayTime, nAddedRankedKillCount, nAddedRankedDeathCount);
...
static bool CheckItemXML_ReadXML(map<unsigned long int, string>& ItemXmlMap, const char* szXmlFile)
{
	...
	rapidxml::xml_document<> doc;
	if (!doc.parse<0>(const_cast<char*>(buffer)))
		return false;
	...
	for (auto itor = rootNode->first_node(); itor; itor = itor->next_sibling())
	{
		...
		if (_stricmp(itor->name(), "item") == 0)
		{
			unsigned long int id;
			id = std::strtoul(itor->first_attribute("id")->value(), 0, 10);
			...
			ItemXmlMap.insert(map<unsigned long int, string>::value_type(id, string(itor->first_attribute("name")->value())));
		}
	}
...
bool MMatchServer::CheckItemXML()
{
	...
	fopen_s(&fp,"shop_allitemtest.xml", "wt");
	if (fp)
	{
...
bool MMatchServer::CheckUpdateItemXML()
{
	...
	for (auto itor = rootNode->first_node(); itor; itor = itor->next_sibling())
	{
		...
		if (_stricmp(itor->name(), "STR") == 0)
		{
			ix tix;

			if (strncmp("ZITEM_NAME_", itor->first_attribute("id")->value(), 11) == 0)
			{
				if (imName.end() != imName.find(itor->first_attribute("id")->value()))
				{
					ASSERT("중복");
					continue;
				}
				tix.id = itor->first_attribute("id")->value();
				tix.name = itor->value();

				imName.insert(map<string, ix>::value_type(itor->first_attribute("id")->value(), tix));

			}

			else if (0 == strncmp("ZITEM_DESC_", itor->first_attribute("id")->value(), 11))
			{
				if (imDesc.end() != imDesc.find(itor->first_attribute("id")->value()))
				{
					ASSERT("중복");
					continue;
				}
				...
				imDesc.insert(map<string, ix>::value_type(itor->first_attribute("id")->value(), tix));
			}
			else
			{
				// ASSERT( 0 && "이상하다...." );
	...
	for( ; it != end; ++it )
	{
		...
		strcpy_s( szID, it->second.id.c_str() + 11 );
...
//		CopyMemory(pCmdBlock, pbyReqMsg, 160);
//
