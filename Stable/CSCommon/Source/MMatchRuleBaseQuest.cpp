...
void MMatchRuleBaseQuest::OnEnterBattle(MUID& uidChar)
{
	...
	for (auto& itor = m_NPCManager.GetNPCObjectMap().begin(); itor != m_NPCManager.GetNPCObjectMap().end(); ++itor)
	{
		...
		if (npcObj == nullptr)
			continue;
		MUID controllingPlayer = MUID(0, 0);
		if (npcObj->GetController().IsInvalid())
			controllingPlayer = uidChar;
		else
		...
		pCmd->AddParameter(new MCmdParamUChar((unsigned char)npcObj->GetType()));
