...
bool MMatchEventFactory::MakeEventList(const EventDataVec& EvnDataVec, EventPtrVec& EvnPtrVec)
{
	...
	for (end = EvnDataVec.end(); it != end; ++it)
	{
		...
		if (0 != pEvent)
		{
			...
			{
				mlog("MMatchEventFactory::MakeEventList - InitFail. ID:%u Name:%s.\n",
...
bool MMatchEventFactoryManager::LoadEventListXML(const string& strFileName, MZFileSystem* pFileSystem)
{
	...
	for (int i = 0; i < nAttrCnt; ++i)
	{
		...
		if (0 == _stricmp(EL_COUNTRY, szAttrName))
		{
			...
			*/
			...
			string strCountry;
			MCountry nCountry = MGetLocale()->GetCountry();
			switch (nCountry)
			{
			case MC_KOREA:
			{
...
void MMatchEventFactoryManager::ParseEvent(MXmlElement& chrElement)
{
	...
	for (int i = 0; i < nAttrCnt; ++i)
	{
		...
		if (0 == _stricmp(EL_EVENTID, szAttrName))
		{
			...
			{
				ASSERT(0 && "Event.xml Fail");
				mlog("MMatchEventFactoryManager::ParseEvent - Event.xml Fail Event ID(%u) Fail.\n",
					dwEventID);
				return;
			}
			continue;
