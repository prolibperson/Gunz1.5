...
protected:
	...
	vector<int>				m_stageMods;//Custom: roomMods
...
public:
	...
	bool isModEnabled(int mod);
	void SetStageMods(vector<int> mods);
	int GetStageModCount() { return (int)m_stageMods.size(); }
	int GetStageMod(int mod) { return m_stageMods.at(mod); }

	vector<int> GetStageMods() { return m_stageMods; }
