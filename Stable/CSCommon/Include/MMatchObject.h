...
constexpr int DEFAULT_CHARINFO_MAXWEIGHT = 100;
constexpr int DEFAULT_CHARINFO_SAFEFALLS = 0;
constexpr float DEFAULT_CHARINFO_BONUSRATE = 0.0f;
constexpr int DEFAULT_CHARINFO_PRIZE = 0;
...
public:
	...
	int					m_nER;
	int					m_nWR;
	unsigned long int	m_nEquipedItemCIID[MMCIP_END], m_nTEquipedItemID[MMCIP_END];
...
public:
	...
	void SetPeerAddr(DWORD dwIP, char* szIP, unsigned short nPort)	{ m_dwIP=dwIP; strcpy_s(m_szIP, sizeof(m_szIP),szIP); m_nPort = nPort; }
...
public:
	MMatchObjectClass GetBlitzClass() { return m_blitzClass; }
	void SetBlitzClass(MMatchObjectClass i) { m_blitzClass = i; }
};
...
const string g_szHairMeshName[MAX_COSTUME_HAIR][2] = 
{
	...
	{"eq_head_hair_003", "eq_head_hair002"},	// This is currently disabled, you can replace with another model.
};

#define MAX_COSTUME_FACE		7
