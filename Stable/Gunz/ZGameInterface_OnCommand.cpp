...
bool ZGameInterface::OnCommand(MCommand* pCommand)
{
	...
	case MC_NEWQUEST_STAGE_GAME_INFO:
		{
				///TODO:????
		}break;
	...
	case MC_MATCH_ACHIEVEMENT_COMPLETED :
		{
			///TODO: handle achievement logic
			MCommandParameter* param = pCommand->GetParameter(0);
			if (param->GetType() != MPT_BLOB)
				break;

			void* achievementBlob = param->GetPointer();
			if (achievementBlob == nullptr)
				break;

			MTD_Achievement achievement = *(reinterpret_cast<MTD_Achievement*>(MGetBlobArrayElement(achievementBlob, 0)));



			ZGetMyInfo()->AddAchievement(achievement);
		}break;
	...
	case MC_ADMIN_REQUEST_SLAP:
		{
			...
			std::string outputMsg = adminObj->GetCharName();//
			outputMsg.append(" Has Slapped ");
			outputMsg.append(playerObj->GetCharName());
