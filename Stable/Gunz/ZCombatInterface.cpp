...
bool ZCombatInterface::OnCreate()
{
	...
	if (!m_AmmoCount->Create("AmmoBar", RGetDevice(), "Interface/default/combat/ammogauge.bmp.dds"))
	{
...
void ZCombatInterface::DrawMyWeaponPont(MDrawContext* pDC)
{
	...
	char szDamageInfo[64];

	// 무기 이름
	TextRelative(pDC,660.f/800.f,490.f/600.f,m_szItemName);

	sprintf(szDamageInfo, "Damage Dealt : %d", ZGetCombatInterface()->GetDamageDealt());
	TextRelative(pDC, 10.f / 800.f, 120.f / 800.f, szDamageInfo);
	...
	{
		// item stats shit
		MMatchItemDesc* pItemDesca = MGetMatchItemDescMgr()->GetItemDesc(pCharacter->GetItems()->GetSelectedWeapon()->GetDescID());
		if (pItemDesca) {
			sprintf(szWeaponInfo, "Damage %d | Delay %d", pItemDesca->m_nDamage.Ref(), pItemDesca->m_nDelay.Ref());
			TextRelative(pDC, 660.f / 800.f, 510.f / 600.f, szWeaponInfo);
		}
	}
	else if (nParts != MMCIP_MELEE && nParts < MMCIP_END) 
	{
		MMatchItemDesc* pItemDesc = MGetMatchItemDescMgr()->GetItemDesc(pCharacter->GetItems()->GetSelectedWeapon()->GetDescID());

		// Ammo bar
		float width = (175.f / static_cast<float>(pCharacter->GetItems()->GetSelectedWeapon()->GetDesc()->m_nMagazine.Ref())) * static_cast<float>(pCharacter->GetItems()->GetSelectedWeapon()->GetBulletCurrMagazine());
		BitmapRelative(pDC, 660.f / 800.f, 470.f / 600.f, width * (RGetScreenWidth() / 1024.f), (RGetScreenHeight() / 768.000f) * 16, m_AmmoCount->GetSourceBitmap());

		// ammo numbers
		sprintf(buffer, "%d / %d", m_nBulletCurrMagazine, m_nBulletSpare);
		TextRelative(pDC, 720.f/800.f, 584.5f/600.f, buffer);
