...
void ZActor::OnUpdate(float fDelta)
{
	...
	//kimyhwan : ↑착지 상태가 제대로 작동하지 않는 것은 m_pModule_Movable->Update()가 한프레임에 두번 실행되기 때문인 것 같다.
	// 지금 ZActor는 m_pModule_Movable->Update()를 직접 실행하는데, ObjectManager가 모든 캐릭터의 모듈을 또 업데이트하기 때문에
	// 한 프레임에 두번 업데이트 하고 있는 셈이다. 그래서 모든 현재 npc는 두배로 빨리 움직이고 있으며 -_-
	// m_pModule_Movable->isLanding()은 착지 순간에만 true가 되었다가 다음 업데이트때는 다시 false가 되는 플래그이므로
	// 결과적으로 종종 isLanding()에서 제대로된 값을 받지 못하는 경우가 생기는 것이다.. 지금와서 고치긴 너무 늦었다. 버스 떠난지 오래.
}
	...
	if(CheckFlag(AF_MY_CONTROL))
	{
		...
		if(bSkipDamagedAnimation==false) {
			...
			if((damageType==ZD_MELEE) || (damageType==ZD_KATANA_SPLASH)) {
				...
				if( m_pNPCInfo->bNeverPushed == false)	// 원거리 공격 타격없는
				{
					if (m_nDamageCount >= 5)
					{
