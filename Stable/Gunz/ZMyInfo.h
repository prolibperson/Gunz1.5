...
struct ZMySystemInfo
{
	...
	unsigned char	pbyAckMsg[56];			// HShield CRC 응답 메세지
	unsigned char	pbyGuidAckMsg[20];	// HShield GUID 응답 메세지
...
public:
	...
	bool				IsAdminGrade() {
		if ( (GetUGradeID() == MMUG_EVENTMASTER) || (GetUGradeID() == MMUG_DEVELOPER) || (GetUGradeID() == MMUG_ADMIN) || GetUGradeID() ==MMUG_STAFF)
			return true;
		else
			return false;
	}
