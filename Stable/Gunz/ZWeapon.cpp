...
#define ROCKET_VELOCITY			2700.f	// 2004³â 9¿ù 21ÀÏÀÚ ÀÌÀüÀº 2500.f
#define ROCKET_SPLASH_RANGE		350.f	// ·ÎÄÏ ½ºÇÃ·¡½Ã ¹üÀ§
#define ROCKET_MINIMUM_DAMAGE	.3f		// ·ÎÄÏ ¹Ì´Ï¸Ø µ¥¹ÌÁö
#define ROCKET_KNOCKBACK_CONST	.5f		// ·ÎÄÏ¿¡ Æ¨°ÜÁ® ³ª°¡´Â »ó¼ö
...
{
	// ÀÚ½ÅÀÇ ÀÌÆåÆ® ±×¸®°í..
	// ÁÖº¯ À¯´Öµé¿¡°Ô µ¥¹ÌÁö ÁÖ´Â ¸Þ½ÃÁö ³¯¸®°í..
	// ( ¸Þ½ÃÁö ¹ÞÀº°÷¿¡¼­ ÁÖº¯ À¯´Öµé Ã¼Å©ÇÏ°í.. 
	// Á×Àº À¯´ÖÀÌ ÀÖ´Ù¸é ¹ß»çÀ¯´ÖÀÇ Å³¼ö¸¦ ¿Ã·ÁÁÖ±â)

	rvector v = m_Position;

	//	if(v.z < 0.f)	v.z = 0.f;//³ôÀÌ

	rvector dir = -RealSpace2::RCameraDirection;
	ZGetEffectManager()->AddRocketEffect(v, dir);

	// ¸Þ½ÃÁö¸¦ ³¯¸®°Å³ª~
	// °¢°¢ÀÇ Å¬¶óÀÌ¾ðÆ®ÀÇ ¹«±â Á¤º¸¸¦ ¹Ï°í Ã³¸®ÇÏ°Å³ª~
...
void ZWeaponItemKit::UpdateWorldItemFloorDrop(DWORD dwPickPassFlag)
{
	...
	if (bPicked && fabsf(Magnitude(rpi.PickPos - m_Position)) < 5.0f) {
		if (m_bWorldItemFloorDrop == false) {
			...
			m_fDeathTime = ZGetGame()->GetTime() + 2.0f;//³×Æ®¿÷ Àü¼Û ½Ã°£°í·Á ÃÖ´ë 2.0ÃÊµÚ¿¡ ÀÚµ¿ ¼Ò¸ê
		}
	}
}
...
void ZWeaponItemKit::UpdatePos(float fElapsedTime, DWORD dwPickPassFlag)
{
	...
	if (bPicked && fabsf(Magnitude(pickpos - m_Position)) < fDist)
	{ // ¾ð´öÀÌ³ª ³­°£À§¿¡¼­ ¶¥¿¡ ¶³¾îÁü(±âº» ¶³¾îÁö´Â ½Ã°£º¸´Ù ¸¹ÀÌ °É¸±°æ¿ì)
		m_Position = pickpos + normal;
		m_Velocity = GetReflectionVector(m_Velocity, normal);
		m_Velocity *= zpi.pObject ? 0.1f : 0.2f;	// Ä³¸¯ÅÍ¿¡ Ãæµ¹ÇÏ¸é ¼Óµµ°¡ ¸¹ÀÌ ÁØ´Ù
		m_Velocity *= 0.2f;

		// º®¹æÇâÀÇ ¼Óµµ¼ººÐÀ» ÁÙÀÎ´Ù.
		Normalize(normal);
...
bool ZWeaponItemKit::Update(float fElapsedTime)
{
	...
	if (m_bWorldItemFloorDrop) // ¿ùµå ¾ÆÀÌÅÛÀÌ ¶¥¿¡ ¶³¾îÁö¸é Ã³¸®ÇÏÁö ¾Ê´Â´Ù.
		return true;

	//	if(m_bSendMsg) { // ¿ÜºÎ¿¡¼­ bDeath ·Î¸¸ Áö¿î´Ù..
	...
	{	// ¿ùµå¾ÆÀÌÅÛÀÌ ¶³¾îÁú ½Ã°£°ú À§Ä¡¸¦ ¹Ì¸® °è»êÈÄ ¼­¹ö¿¡ ¸Þ¼¼Áö¸¦ ¹Ì¸® º¸³»ÁØ´Ù....20090213 by kammir
		rvector vTempPos = m_Position;
		rvector vTempVelocity = m_Velocity;

		if (m_bSendMsg == false)
		{ // ¸Þ¼¼Áö¸¦ ¾Èº¸³ÂÀ»¶§¿¡¸¸ ½ÇÇà... ÇÑ¹ø¸¸ Àû¿ëµÅ°Ú±ÝÇÑ´Ù.
			float fTick = 0.02f;
			for (int i = 0; i < 500; i++)
			{ // 10ÃÊ Á¤µµ¸¦ ¹Ì¸® ¿¹»óÇÏ¿© ¾ÆÀÌÅÛÀÌ ¶³¾îÁö´Â°É È®ÀÎÇÑ´Ù.
				m_Velocity.z -= 500.f * fTick;
				UpdatePos(fTick, dwPickPassFlag);		// ¾ÆÀÌÅÛÀÌ °è¼Ó ¶³¾îÁø´Ù.
				UpdateWorldItemFloorDrop(dwPickPassFlag);	// ¾ÆÀÌÅÛÀÌ ¶¥¿¡ ¶³¾îÁ³´ÂÁö È®ÀÎ
			...
			// ¸¸¾à 10ÃÊ°¡ Áö³ªµµ ¹Ù´Ú¿¡ ¶³¾îÁöÁö ¾Ê¾ÒÀ¸¸é ¾ÆÀÌÅÛÀ» ±×³É Á×¿©ÁØ´Ù.
			m_bSendMsg = true;
		}

		m_Position = vTempPos;			// ¸Þ¼¼Áö¸¦ º¸³»±âÀ§ÇØ ¹Ì¸® °è»êÇÑ º¯°æµÈ °ªÀ» ¿ø·¡´ë·Î µ¹·ÁÁØ´Ù.
		m_Velocity = vTempVelocity;
	}

	// ¿ùµå¾ÆÀÌÅÛÀÌ ¶³¾îÁö´Â°É È­¸é¿¡ º¸¿©Áú¼öÀÖ°Ô °è»êÇØÁØ´Ù.
	m_Velocity.z -= 500.f * fElapsedTime;		// ½Ã°£¿¡ µû¸¥ ¶¥¿¡ ¶³¾îÁö´Â ¼Óµµ
	UpdatePos(fElapsedTime, dwPickPassFlag); // ¼Óµµ¿Í ¿ùµå¾ÆÀÌÅÛÀÇ ³ôÀÌ¸¦ ½Ã°£´ç °è»êÇÔÀ¸·Î½á ¾Æ·¡·Î ¶³¾îÁø´Ù.
	UpdateWorldItemFloorDrop(dwPickPassFlag);	// ¾ÆÀÌÅÛÀÌ ¶¥¿¡ ¶³¾îÁ³´ÂÁö È®ÀÎ(¶¥¹Ù´ÚÇÏ°í ¿ùµå¾ÆÀÌÅÛÀÇ ³ôÀÌ°¡ 5¹Ì¸¸ÀÌ¸é Ã³¸®)
...
void ZWeaponGrenade::Create(RMesh* pMesh, rvector &pos, rvector &velocity, ZObject* pOwner) {
	...
	m_nSoundCount = rand() % 2 + 2;	// 2~ 3¹ø ¼Ò¸®¸¦ ³½´Ù
}


#define GRENADE_LIFE			2.f		// 3ÃÊ µÚ¿¡ ÅÍÁü
...
bool ZWeaponGrenade::Update(float fElapsedTime)
{
...

	{
			...
			*/
		{
			m_Position = pickpos + normal;
			m_Velocity = GetReflectionVector(m_Velocity, normal);
			m_Velocity *= zpi.pObject ? 0.4f : 0.8f;		// Ä³¸¯ÅÍ¿¡ Ãæµ¹ÇÏ¸é ¼Óµµ°¡ ¸¹ÀÌ ÁØ´Ù
...
#define FLASHBANG_LIFE	3	//<<- TODO : ¹«±âÀÇ ¼Ó¼º¿¡¼­ ¹Þ¾Æ¿À±â
#define FLASHBANG_DISTANCE	2000		// 20¹ÌÅÍ ±îÁö ¿µÇâÀ» ÁÜ
...
void ZWeaponFlashBang::Explosion()
{
	...
	if (dist > FLASHBANG_DISTANCE)	// °Å¸® ¹Û¿¡¼­´Â ¿µÇâÀ» ÁÖÁö ¸øÇÔ
	{
		return;
	}

	if (ZGetGame()->m_pMyCharacter->IsDie())	// ¿ÉÀú¹ö ¸ðµå¿¡¼­´Â ¿µÇâÀ» ¹ÞÁö ¾ÊÀ½
	{
		return;
	}
...
#define MAGIC_WEAPON_LIFE			10.f		// 10ÃÊ µÚ¿¡ ÅÍÁü
...
#define ANGULAR_VELOCITY	0.01f		// °¢¼Óµµ : ÃÊ´ç È¸Àü°¡´ÉÇÑ °¢ÀÇ Å©±â. (´ÜÀ§:¶óµð¾È)
...
#define MAGIC_KNOCKBACK_CONST   .3f	
	...
	if (type == WMET_OBJECT) {// object ¿¡ ºÎµúÈ÷´Â °æ¿ì...
		float fScale = m_fMagicScale;

		// ÀÌÆåÆ®´Â ½ºÄÉÀÏ¸µÇÏ¸é ÁÙÀÎ´Ù.
...
constexpr float DYNAMITE_LIFE = 0.7f;	// Ã¹¹øÂ° Ãæµ¹ ÈÄ 0.7ÃÊÈÄ¿¡ ÅÍÁü
...
bool ZWeaponDynamite::Update(float fElapsedTime)
{
	...
	// Ã³À½À¸·Î ºÎµúÈù ¶§ºÎÅÍÀÇ ½Ã°£À» Ã¼Å©ÇÏ¿© Æø¹ß
	if (m_bCollided &&
...

	{
		...
		if (bPicked && fabsf(Magnitude(pickpos - m_Position)) < fDist)
		{
			// Ã³À½À¸·Î ¾îµò°¡ ºÎµúÇûÀ» ¶§ºÎÅÍ ½Ã°£À» Àç±â ½ÃÀÛ
			if (!m_bCollided)
			{
				m_bCollided = true;
				m_fStartTime = ZGetGame()->GetTime();
...
bool ZWeaponTrap::Create(RMesh* pMesh, rvector& pos, rvector& velocity, int nItemId, ZObject* pOwner)
{
	...
	if (!pOwner->IsNPC())
	{
		...
		// ¼­¹ö¿¡ Æ®·¦À» ´øÁø´Ù´Â °ÍÀ» º¸³½´Ù. ¼­¹ö´Â ¹º°¡ ´øÁ®Á³´Ù´Â °Í¸¸ ±â¾ïÇØµÎ¾ú´Ù°¡ 
		// ÀÌÈÄ¿¡ Æ®·¦ÀÇ ¹ßµ¿ Åëº¸¸¦ ´Ù½Ã ¹ÞÀ»¶§ ¾Æ ¾Æ±î ´øÁ®Áø ¾Ö°¡ ¾îµð¿¡¼­ ¹ßµ¿µÆ±¸³ª ÇÏ°í Ã¼Å©ÇÏ¸é µÈ´Ù.
		// ¾î¶² Æ®·¦ÀÌ ´øÁ®Á³À¸³ª ¾ÆÁ÷ ¹ßµ¿µÇÁö ¾ÊÀº »óÅÂÀÏ ¶§ ³­ÀÔÇÑ À¯Àú¿¡°Ô´Â ÀÌ ¹ßµ¿µÈ Æ®·¦¿¡ ´ëÇØ¼­ ¼­¹ö°¡ º°µµ·Î ¸Þ½ÃÁö¸¦ º¸³»ÁØ´Ù
...
#define TRAP_LANDING_TIME 0.7f		// ¶¥¿¡ ´êÀº ÈÄ ÀÌ ½Ã°£µ¿¾È °¡¸¸È÷ ±â´Ù¸°´Ù
...
bool ZWeaponTrap::Update(float fElapsedTime)
{
...

	{
		...
		if (bPicked && fabsf(Magnitude(pickpos - m_Position)) < fDist)
		{
			...
			// Áö¸é¿¡ ´ê¾ÒÀ» ¶§ ÀÏÁ¤½Ã°£ ÈÄ Æø¹ß (º®ÀÌ³ª ÃµÀå Ãæµ¹Àº ÆÐ½º)
			if (DotProduct(normal, rvector(0, 0, 1)) > 0.8f)
			{
				if (!m_bLanded)
				{
					...
					// Æ®·¦ÀÌ ¼³Ä¡µÈ À§Ä¡¸¦ ¾Ë ¼ö ÀÖ°Ô °¡ÀÌµå¸¦ Á¶±×¸Ä°Ô Ç¥½ÃÇÏÀÚ
...
void ZWeaponTrap::Activate()
{
	...
	// ¼­¹ö¿¡ ¹ßµ¿µÈ Æ®·¦À» µî·ÏÇØ¼­ ÀÌÈÄ ³­ÀÔÇÏ´Â À¯Àú°¡ ¿ùµå¿¡ ¼³Ä¡µÈ Æ®·¦µéÀ» ¾Ë ¼ö ÀÖµµ·Ï ÇÏÀÚ
	if (m_uidOwner == ZGetMyUID())
...
{
	// ¹ßµ¿ÀÌ Áö¼ÓµÉ µ¿¾È º¸¿©Áú ÀÌÆåÆ® ¼¼ÆÃ
...
bool ZWeaponTrap::UpdateOnActivated(float fElapsedTime)
{
	if (m_bActivated)
	{
		...
		{
			m_fNextEffectTime += 1.f;	// 1ÃÊ¸¶´Ù ÀÌÆåÆ®¸¦ Ãâ·ÂÇÑ´Ù
...
bool ZWeaponStunGrenade::Update(float fElapsedTime)
{
	...
	// Ã³À½À¸·Î ºÎµúÈ"¶§ºÎÅÍÀÇ ½Ã°£À» Ã¼Å©ÇÏ¿© Æø¹ß
	if (m_bCollided &&
		ZGetGame()->GetTime() - m_fStartTime > DYNAMITE_LIFE)
	{
		Explosion();
