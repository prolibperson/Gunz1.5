...
void ZOptionInterface::InitInterfaceOption(void)
{
	...
	*/
	...
	{	
		...
		if(pEdit)	{
			pEdit->SetText(std::to_string(Z_ETC_FRAMELIMIT_PERSECOND).c_str());
			RSetFrameLimitPerSeceond(Z_ETC_FRAMELIMIT_PERSECOND);
		}

		pEdit = (MEdit*)pResource->FindWidget("FOV");
		if (pEdit) {
			pEdit->SetText(std::to_string(Z_ETC_FOV).c_str());
			if(Z_ETC_FOV <= 120.f && Z_ETC_FOV >= 30.f)
				g_fFOVAsd = Z_ETC_FOV;
		}
...
//		ZGetInput()->UnregisterActionKey(i);
	...
	*/
	...
	{	// Etc
		...
		{
			GunzState state = ZApplication::GetGameInterface()->GetState();
			Z_ETC_FOV = atoi(pEdit->GetText());
			if (state == GUNZ_GAME) {
				if (Z_ETC_FOV <= 120.f && Z_ETC_FOV >= 30.f)
					g_fFOV = D3DXToRadian(Z_ETC_FOV);
				else
					g_fFOV = D3DXToRadian(70);
			}
			else {
				if (Z_ETC_FOV <= 120.f && Z_ETC_FOV >= 30.f)
					g_fFOVAsd = Z_ETC_FOV;
