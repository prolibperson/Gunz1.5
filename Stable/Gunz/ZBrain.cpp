...
void ZBrain::ProcessBuildPath( float fDelta)
{
	...
	//custom: jetman quest fix
	rvector nozmypos = m_pBody->GetPosition(); nozmypos.z = 0;
	m_WayPointList.clear();
	for (list<rvector>::iterator itor = navMesh.GetWaypointList().begin(); itor != navMesh.GetWaypointList().end(); ++itor) {
		rvector wayposnoz = (*itor); wayposnoz.z = 0;
		if ((float)Magnitude(nozmypos - wayposnoz) >= 20.f)
