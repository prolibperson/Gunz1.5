...
float adjustDistByRatio(float normalAspect, float realAspect)
{
	float adjustedAspect = 1;

	if (normalAspect != realAspect)
		adjustedAspect = normalAspect / realAspect;

	return CAMERA_DEFAULT_DISTANCE * adjustedAspect;
}

void ZCamera::AdjustDist()
{
	float screenHeight = static_cast<float>(RGetScreenHeight());
	float screenWidth = static_cast<float>(RGetScreenWidth());
	if (screenHeight / screenWidth != 0.75f)
	{
		m_fDist = adjustDistByRatio(0.75f, screenHeight / screenWidth);
	}
}