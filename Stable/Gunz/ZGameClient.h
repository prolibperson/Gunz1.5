...
protected:
	...
	void CheckStageJoinMods(const MUID& player);
};
...
#define ZPOSTCMD8(_ID, _P0, _P1, _P2, _P3, _P4, _P5, _P6, _P7) {MCommand* pC=ZNewCmd(_ID); pC->AP(_P0); pC->AP(_P1); pC->AP(_P2); pC->AP(_P3); pC->AP(_P4); pC->AP(_P5); pC->AP(_P6); pC->AP(_P7); ZPostCommand(pC); }
