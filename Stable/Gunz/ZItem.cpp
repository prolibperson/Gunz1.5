...
bool ZItem::Reload()
{
	...
	if (!ZGetGameClient()->GetMatchStageSetting()->isModEnabled((int)SM_IA))
...
bool ZItem::isReloadable()
{
	...
	if (m_nBulletSpare.Ref() == 0 && m_nBulletCurrMagazine.Ref() > 0)
		return true;
...
float ZItem::GetPiercingRatio(MMatchWeaponType wtype, RMeshPartsType partstype)
{
	...
	switch(wtype) {
		...
		case MWT_REVOLVERx2:
			...
			if (ZGetGame() && ZGetGameClient()->GetMatchStageSetting()->isModEnabled(SM_LEAD))
			{
