...
bool ZMapSpawnManager::Add(ZMapSpawnData* pMapSpawnData)
{
	...
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, "spawn_blitz_barricade_r", strlen("spawn_blitz_barricade_r")))
	{
		pMapSpawnData->m_nType = ZMST_SPAWN_BARRICADE_RED;
		m_SpawnArray[ZMST_SPAWN_BARRICADE_RED].push_back(pMapSpawnData);
	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, "spawn_blitz_barricade_b", strlen("spawn_blitz_barricade_b")))
	{
		pMapSpawnData->m_nType = ZMST_SPAWN_BARRICADE_BLUE;
		m_SpawnArray[ZMST_SPAWN_BARRICADE_BLUE].push_back(pMapSpawnData);
	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, "spawn_blitz_guardian", strlen("spawn_blitz_guardian")))
	{
		pMapSpawnData->m_nType = ZMST_SPAWN_GUARDIAN;
		m_SpawnArray[ZMST_SPAWN_GUARDIAN].push_back(pMapSpawnData);

	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, "spawn_blitz_radar_r", strlen("spawn_blitz_radar_r")))
	{
		pMapSpawnData->m_nType = ZMST_SPAWN_RADAR_RED;
		m_SpawnArray[ZMST_SPAWN_RADAR_RED].push_back(pMapSpawnData);
	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, "spawn_blitz_radar_b", strlen("spawn_blitz_radar_b")))
	{
		pMapSpawnData->m_nType = ZMST_SPAWN_RADAR_BLUE;
		m_SpawnArray[ZMST_SPAWN_RADAR_BLUE].push_back(pMapSpawnData);
	}
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, ZTOK_SPAWN_TRESURE, strlen(ZTOK_SPAWN_TRESURE)))
	{
		pMapSpawnData->m_nType = ZMST_SPAWN_TRESURE;
		m_SpawnArray[ZMST_SPAWN_TRESURE].push_back(pMapSpawnData);
	}
	//TODO: Determine how to handle this in relation to runwaypointsalongroute
	else if (!strnicmp(pMapSpawnData->m_szSpawnName, ZTOK_DUMMY_ROUTE, strlen(ZTOK_DUMMY_ROUTE)))
	{
		pMapSpawnData->m_nType = ZMST_ROUTE;
		m_SpawnArray[ZMST_ROUTE].push_back(pMapSpawnData);
	}
