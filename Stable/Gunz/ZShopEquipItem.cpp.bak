...
int ZShopEquipItem::GetPrice()
{
	_ASSERT(!(m_pHandlerPurchase && m_pHandlerSell));	// 두가지가 동시에 존재한다는 것은 뭔가 잘못되었다는 것

	int nPrice(0);
	if (m_pHandlerPurchase) 
		m_pHandlerPurchase->GetPrice(nPrice);
	else if (m_pHandlerSell)
	{
...
const char* IShopEquipItemHandle_Purchase::GetPriceText( char* szBuf )
{
	...
	sprintf(sz, "%d %s", nPrice, ZMsg(MSG_CHARINFO_BOUNTY)); // [7 days] 20 BP / 65 LC
	strcat(szBuf, sz);
...
bool ZShopEquipItemHandle_PurchaseGamble::GetPrice( int& out_nPrice )
{
...
bool ZShopEquipItemHandle_PurchaseGamble::GetRentalHour( int& out_nRentalHour )
{
	if (!m_pItem || !m_pItem->GetDesc()) { _ASSERT(0); return false; }

	if (m_pItem->GetDesc()->IsCash() == true)
		out_nRentalHour = 168;
	else
		out_nRentalHour = 0;
...
bool ZShopEquipItemHandle_PurchaseMatch::GetPrice( int& out_nPrice )
{
	if (!m_pItem || !m_pItem->GetDesc()) { _ASSERT(0); return false; }
...
bool ZShopEquipItemHandle_PurchaseMatch::GetRentalHour( int& out_nRentalHour )
{
	if (!m_pItem || !m_pItem->GetDesc()) { _ASSERT(0); return false; }
...
bool ZShopEquipItemHandle_PurchaseQuest::GetPrice( int& out_nPrice )
{
	if (!m_pItem || !m_pItem->GetDesc()) { _ASSERT(0); return false; }
...
bool ZShopEquipItemHandle_PurchaseQuest::GetRentalHour( int& out_nRentalHour )
{
	if (!m_pItem || !m_pItem->GetDesc()) { _ASSERT(0); return false; }
...
const char* IShopEquipItemHandle_Sell::GetPriceText( char* szBuf )
{
	...
	char sz[256];
	sprintf(sz, "%d %s", nPrice, ZMsg(MSG_CHARINFO_BOUNTY));
	strcat(szBuf, sz);
