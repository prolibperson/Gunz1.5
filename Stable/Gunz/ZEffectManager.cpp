...
//	m_pEBSBulletMark[1] = new ZEffectBillboardSource("SFX/bullet-mark02.tga");
	...
	m_pRangeDamaged[6] = m_pEffectMeshMgr->Get("ef_damage07.elu"); //will be used by challengequest in the future
...
//	m_pRocketSmokeEffect		= m_pEffectMeshMgr->Get("rocket_smoke_effect");
	...
	m_pFlagEffect[0] = m_pEffectMeshMgr->Get("ef_wctf_b");
	m_pFlagEffect[1] = m_pEffectMeshMgr->Get("ef_wctf_r");
//	m_pLighteningEffect = m_pEffectMeshMgr->Get("ef_lightening");
	m_pBlizzardEffect = m_pEffectMeshMgr->Get("ef_blizzard");
	//Custom: blitzkrieg
	m_pBlitzEffect[0] = m_pEffectMeshMgr->Get("ef_Blitz_LessHonor_Gain");
	m_pBlitzEffect[1] = m_pEffectMeshMgr->Get("ef_Blitz_LegularHonor_Gain");
	m_pBlitzEffect[2] = m_pEffectMeshMgr->Get("ef_Blitz_MoreHonor_Gain");
	m_pBlitzEffect[3] = m_pEffectMeshMgr->Get("ef_Blitz_BarricadeBuff");
...
bool ZEffectManager::GetEffect(ZEffect* currEffect)
{
	return false;
}
...
float ZEffectManager::GetEnchantDamageObjectSIze(ZObject* pObj)
{
	...
	ZActorBase* pActor = MDynamicCast(ZActorBase,pObj);
	...
	if(pActor) { 
		...
		fSize = pActor->GetCollideRadius() / 35.f;
		fSize *= fSize;
	}
...
#define SMOKE_VELOCITY				110.f
		...
		if(bRender) {
			...
			if(pCObj)
			{
				if(wtype!=MWT_SHOTGUN && pCObj->IsHero() && bSlugOutput == true)	// 샷건의 한알 한알은 탄피 생략
				{
...
void ZEffectManager::AddBlitzBuff(ZCharacter* pChar, BLITZHONOREFFECT effect)
{
	if (effect < BLITZHONOREFFECT::BE_LESSGAIN || effect >= BLITZHONOREFFECT::BE_END)
		return;

	if (!pChar->GetInitialized()) return;
	if (!pChar->GetVisualMesh()->IsRender()) return;
	
	ZEffect* pNew = 
	new ZEffectLevelUp(m_pBlitzEffect[effect], const_cast<D3DXVECTOR3&>(pChar->GetPosition())
		,const_cast<D3DXVECTOR3&>(pChar->GetDirection()),rvector(0,0,0),static_cast<ZObject*>(pChar));
	((ZEffectLevelUp*)pNew)->SetAlignType(1);
	((ZEffectLevelUp*)pNew)->m_type = eq_parts_pos_info_Spine2;
...
{
	///TODO: create jjang itemslot
	//if (pObj->GetItems()->GetItem(MMCIP_JJANG) == nullptr)
	//	return;
	ZEffect* pNew = NULL;
...
void ZEffectManager::AddItemTrapEffect(rvector& Target, rvector& TargetNormal)
{
	...
	{		
		rvector up = rvector(0.f,0.f,1.f);	// 타겟 폭발 흔적
