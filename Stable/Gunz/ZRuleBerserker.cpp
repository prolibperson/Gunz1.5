...
void ZRuleBerserker::AssignBerserker(MUID& uidBerserker)
{
	...
	ZCharacter* pBerserkerChar = (ZCharacter*) ZGetGame()->m_CharacterManager.Find(uidBerserker);
	if (pBerserkerChar)
	{
...
void ZRuleBerserker::OnUpdate(float fDelta)
{
	...
	if (BERSERKER_UPDATE_HEALTH_TIME < m_fElapsedHealthUpdateTime)
	{
		...
		ZCharacter* pBerserker = (ZCharacter*) ZGetGame()->m_CharacterManager.Find(m_uidBerserker);
