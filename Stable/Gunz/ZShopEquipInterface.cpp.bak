...
bool ZShopEquipInterface::Equip(void)
{
	...
	if (ZSEIT_GAMBLE == pListItem->GetItemData()->GetType())
	{
		...
		if (pGItemDesc)
		{
			...
			if( ZGetGameClient()->GetServerMode() != MSM_CLAN && 1000100 <= pGItemDesc->GetGambleItemID()  && pGItemDesc->GetGambleItemID() < 1000200)
			{
...
bool ZShopEquipInterface::isWeaponAllowed(const MMatchItemDesc* pItemDesc)
{
	...
	if (ZGetGameClient()->GetMatchStageSetting()->isModEnabled(SM_NA)) {
		if (pItemDesc->m_nType.Ref() == MMIT_EQUIPMENT || pItemDesc->m_nType.Ref() == MMIT_AVATAR)
			return false;
	}
...
/*#ifndef _DEBUG
	...
	// Set sub filter
	pComboBox = (MComboBox*)pResource->FindWidget( "Shop_SubEquipmentFilter");
	if(pComboBox) {
		int sel = pComboBox->GetSelIndex();
	...
	else if ( nTabIndex == 2)
	{
	 ...
	 if (pButton != NULL)
	  pButton->Show(nTabIndex!=0 ? true : false); // true : false
	 pButton = (MButton*)pResource->FindWidget("MyAllEquipmentListCaller");
	 if (pButton != NULL)
	  pButton->Show(nTabIndex!=1 ? true : false); // true : false
...
void ZShopEquipInterface::SelectEquipmentTab(int nTabIndex)
{
	...
	// Set subfilter
	pComboBox = (MComboBox*)pResource->FindWidget( "Equip_SubEquipmentFilter");
	if(pComboBox) {
		int sel = pComboBox->GetSelIndex();
...
void ZShopEquipInterface::SelectEquipmentFrameList( const char* szName, bool bOpen)
{
	...
	if(pPicture != NULL) {
		//if( bOpen && GetArmorWeaponTabIndex() == 0 )  { pPicture->Show(true); }  // COMMENTED DUE TO IN THE NEW SHOP THERE IS NOT CLOSE/OPEN FRAME
	...
	pPicture = (MPicture*)pResource->FindWidget( szTemp);
	if(pPicture != NULL) {
		//if( bOpen && GetArmorWeaponTabIndex() == 1 )  { pPicture->Show(true); }  // COMMENTED DUE TO IN THE NEW SHOP THERE IS NOT CLOSE/OPEN FRAME
		/*else										{*/ pPicture->Show(false);//}		
	}
	...
	pButton = (MButton*)pResource->FindWidget( szTemp); // COMMENTED DUE TO IN THE NEW SHOP THERE IS NOT CLOSE/OPEN FRAME
	...
	*/
	...
	if (itemSlot) {
		...
		int nWidth;
	//	if ( bOpen) nWidth = 220.0f * (float)RGetScreenWidth() / 800.0f; // COMMENTED DUE TO IN THE NEW SHOP THERE IS NOT CLOSE/OPEN FRAME
...
void ZShopEquipInterface::DrawCharInfoText(char* szShopOrEquip, int nReqLevel, int nNewWT, int nNewMaxWT, int nNewHP, int nNewAP, int nReqBounty)
{
	if (szShopOrEquip == NULL) 
	{
		DrawCharInfoText("Shop", nReqLevel, nNewWT, nNewMaxWT, nNewHP, nNewAP, nReqBounty);
		DrawCharInfoText("Equip", nReqLevel, nNewWT, nNewMaxWT, nNewHP, nNewAP, nReqBounty);
		return;
	}
