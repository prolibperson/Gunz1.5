...
//	int			nSetBacks;
	...
	void AddWepCount(int nAddedScores = 1) { nWepCount += nAddedScores; }
	void MinusWepCount(int nAddedScores = 1) { nWepCount -= nAddedScores; }
	//void AddSetbacks(int nAddedScores = 1) { nSetBacks += nAddedScores; }
...
struct ZCharaterStatusBitPacking { 
	union {
		struct {
			...
			bool	m_bTumble:1;			// Trampoline is one of the
			bool	m_bBlast:1;				// When Jim afloat ( When climb )
			bool	m_bBlastFall:1;			// 띄워져서 떨어질때
			bool	m_bBlastDrop:1;			// 떨어지다 땅에 튕길때
			bool	m_bBlastStand:1;		// 일어날때
			bool	m_bBlastAirmove:1;		// After an aerial spin landed
			bool	m_bSpMotion:1;
			bool	m_bCommander:1;			///< Large intestine
		...
		//	bool	m_bCharged:1;			// 힘모인상태
			...
			bool	m_bTagger:1;					///< 술래
			bool	m_isPlantingBomb:1;
			bool    m_isBombPlanted:1;
		};
...
public:
	...
	void InitProperties();
	void SetInfected(bool b) { m_bInfected = b; }
	bool IsInfected() { return m_bInfected; }
	int GetHonor() { return m_Honor; }
	void AddHonor(int n) { m_Honor += n; }
	void ReduceHonor(int n) { m_Honor -= n; }
	void SetBlitzClass(MMatchObjectClass n) { m_blitzClass = n; }
	const MMatchObjectClass GetBlitzClass() const {return m_blitzClass;}
	bool isBombPlanted() { return m_dwStatusBitPackingValue.Ref().m_isBombPlanted; }
	void setBombPlanted(bool b) { m_dwStatusBitPackingValue.Ref().m_isBombPlanted = b; }
...
//	virtual void RegisterModules();
	...
	void CalcBlitzDelay(DWORD& nWeaponDelay, MMatchWeaponType& weaponType);
...
//	bool Move(rvector &diff);
	...
	virtual void OnChangeWeapon(MMatchItemDesc* pWeapon);
...
//	void OnHeal(ZCharacter* pAttacter,int type,int heal);
	...
	void ChangeWeapon(MMatchCharItemParts nParts,bool b = true);
	...
	const char *GetCharName()			{ return m_pMUserAndClanName->Ref().m_szUserName;	}		// 운영자는 처리됨
...
//	float GetMeleeWeaponRange();
	...
	bool Load(MTD_CharInfo* pCharInfo);	// 나중에 MZFile * 로 포팅
	...
	unsigned long int nSpymode_LastWeapons;
...
//	virtual void OnDamage(int damage, float fRatio = 1.0f);
	...
	//Cusstom: antilead ondamaged
	void OnDamagedAntilead(ZObject* pAttacker, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage,float piercingRatio);
