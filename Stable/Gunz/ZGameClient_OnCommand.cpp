		...
		case MC_MATCH_RESPONSE_STAGESETTING:
			{
				...
				MCommandParameter* pStageModParam = pCommand->GetParameter(5);
				if (pStageModParam->GetType() != MPT_BLOB)
					break;

				void* stageModBlob = pStageModParam->GetPointer();
				int count = MGetBlobArrayCount(stageModBlob);
				for (int i = 0; i < count; ++i) {
					int mod = *(int*)MGetBlobArrayElement(stageModBlob, i);
					stageMods.push_back(mod);
				}

				ZGetGameClient()->GetMatchStageSetting()->SetStageMods(stageMods);

				OnResponseStageSetting(uidStage, pStageBlob, nStageCount, pCharBlob, nCharCount, STAGE_STATE(nStageState), uidMaster);

				CheckStageJoinMods(GetPlayerUID());
...
//				pCommand->GetParameter(szProposerCharName,	1, MPT_STR);
		...
		case MC_ADMIN_RESPONSE_MUTE_PLAYER:
			...
			break;
		//Custom: Creaet account from login
		case MC_MATCH_RESPONSE_CREATE_ACCOUNT:
		{
			...
			bool cmd2 = false;
			pCommand->GetParameter(&cmd2, 1, MPT_BOOL);



			ZGetGameInterface()->ShowMessage(szMessage);
			...
			if (cmd2 == true)
			{
				...
				if (pServer == nullptr)
				{
					pServer = pServerList->GetFirstServer();
					if (pServer == nullptr)
						break;
				}
				...
				ZPostLogin(const_cast<char*>(pUsernameEdit->GetText()), const_cast<char*>(pPassEdit->GetText()), szEncryptMD5Value);
			}
		}
		break;//end
