		...
		if(MWidget::IsMsg(szMessage, MBTN_CLK_MSG)==true)
		{
			...
			if ( pServer)
			{
				...
				g_OnConnectCallback = []()
				{
					...
					strcpy_s(szID, pWidget->GetText());
					pWidget = pResource->FindWidget("LoginPassword");
					if (pWidget == NULL) return;
					strcpy_s(szPassword, pWidget->GetText());
					...
					ZPostLogin(szID, szPassword, szEncryptMD5Value);

					MButton* saveButton = (MButton*)pResource->FindWidget("LoginRememberPass");
					if (saveButton && saveButton->GetCheck())
					{
						ZGetApplication()->SetSystemValue("LoginPass", szPassword);
					}
					else if (saveButton && !saveButton->GetCheck())
					{
						ZGetApplication()->SetSystemValue("LoginPass", "");
					}
				};
...
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage) {
		if (MWidget::IsMsg(szMessage, MBTN_CLK_MSG) == true) {
			...
			MEdit* pPassEditTmp = (MEdit*)pResource->FindWidget("AccountPasswordTemp");

			if (!pUsernameEdit || !pPassEdit || !pPassEditTmp)
				return true;

			auto szUsername = pUsernameEdit->GetText();
			auto szPassword = pPassEdit->GetText();
			auto szPasswordTmp = pPassEditTmp->GetText();

			if (strlen(szUsername) < 4)
			{
				ZGetGameInterface()->ShowMessage("Username too short");
				return true;
			}

			if (strlen(szPassword) < 6)
			{
				ZGetGameInterface()->ShowMessage("Password too short");
				return true;
			}

			// 			if (strlen(szPasswordTmp) < 6)
			// 			{
			// 				ZGetGameInterface()->ShowErrorMessage("Please confirm your password correctly");
			// 				return true;
			// 			}

			if (!_stricmp(szPassword, szPasswordTmp) == 0)
			{
				ZGetGameInterface()->ShowMessage("The password doesn't match between each other");
				return true;
			}
			...
			g_OnConnectCallback = [pResource]()
			{
				...
				if (!pUsernameEdit || !pPassEdit || !pEmailEdit) return;

				auto szUsername = pUsernameEdit->GetText();
				auto szPassword = pPassEdit->GetText();
				auto szEmail = pEmailEdit->GetText();

				ZPostCreateAccount(szUsername, szPassword, szEmail);
			};
			...
			if (!g_bConnected)
			{
				...
				if (!TryConnect())
				{
					//mlog("No server avaliable atm\n");
					ZGetGameInterface()->ShowMessage("Couldn't find server to register on");
				}
...
BEGIN_IMPLEMENT_LISTENER(ZGetStageCreateBtnListener, MBTN_CLK_MSG)
	...
	//stagemods
	{
		//Buttons:
		{
			MButton* button = (MButton*)pResource->FindWidget("stagemod_reload");
			if (button && button->GetCheck())
				mods.push_back(SM_IR);

			button = (MButton*)pResource->FindWidget("stagemod_infammo");
			if (button && button->GetCheck())
				mods.push_back(SM_IA);

			button = (MButton*)pResource->FindWidget("stagemod_noswords");
			if (button && button->GetCheck())
				mods.push_back(SM_NS);

			button = (MButton*)pResource->FindWidget("stagemod_noguns");
			if (button && button->GetCheck())
				mods.push_back(SM_NG);

			button = (MButton*)pResource->FindWidget("stagemod_noarmor");
			if (button && button->GetCheck())
				mods.push_back(SM_NA);

			button = (MButton*)pResource->FindWidget("stagemod_noitems");
			if (button && button->GetCheck())
				mods.push_back(SM_NI);

			button = (MButton*)pResource->FindWidget("stagemod_norings");
			if (button && button->GetCheck())
				mods.push_back(SM_NR);

			button = (MButton*)pResource->FindWidget("stagemod_nomassives");
			if (button && button->GetCheck())
				mods.push_back(SM_NM);

			button = (MButton*)pResource->FindWidget("stagemod_lead");
			if (button && button->GetCheck())
				mods.push_back(SM_LEAD);
		}
	...
	void* stageModList = MMakeBlobArray(sizeof(int), mods.size());
	...
	else
	{
		...
		ZPostStageCreate(ZGetGameClient()->GetPlayerUID(), szStageName, bPrivate, szStagePassword,stageModList);
		MEraseBlobArray(stageModList);
	}
END_IMPLEMENT_LISTENER()
...
END_IMPLEMENT_LISTENER()
BEGIN_IMPLEMENT_LISTENER(ZGetAllLadderEquipmentListCallerButtonListener, MBTN_CLK_MSG)
...
BEGIN_IMPLEMENT_LISTENER(ZGetBlitzBlueHunterListener, MBTN_CLK_MSG)
{
	...
	if (ZGetGame()->m_pMyCharacter->GetBlitzClass() == MOC_HUNTER)
		return false;
...
//Custom : Blitzkrieg
///TODO: handle interfacelisteners for selecting class
BEGIN_IMPLEMENT_LISTENER(ZGetBlitzRedHunterListener, MBTN_CLK_MSG)
{
	if (ZGetGame() == nullptr)
		return false;
...
BEGIN_IMPLEMENT_LISTENER(ZGetBlitzBlueTerroristListener, MBTN_CLK_MSG)
{
	...
	if (ZGetGame()->m_pMyCharacter->GetBlitzClass() == MOC_TERRORIST)
		return false;
	...
	for (auto itor = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		if (player->GetTeamID() == ZGetGame()->m_pMyCharacter->GetTeamID()) {
			//if they somehow enable the button, return false, if they get past this, th eserver will catch them???? TODO: determine best way of doing this
			if (player->GetBlitzClass() == MOC_TERRORIST)
				return false;

			ZPostRequestBlitzClass(MOC_TERRORIST);
		}
	}
}
END_IMPLEMENT_LISTENER();
...
//Custom : Blitzkrieg
///TODO: handle interfacelisteners for selecting class
BEGIN_IMPLEMENT_LISTENER(ZGetBlitzRedTerroristListener, MBTN_CLK_MSG)
{
	if (ZGetGame()->m_pMyCharacter->GetTeamID() == MMT_BLUE)
		return false;
...
//Custom: Achievements
BEGIN_IMPLEMENT_LISTENER(ZGetAchievementCallerButtonListener,MBTN_CLK_MSG)
	ZGetGameInterface()->ShowAchievementDialog(true);
END_IMPLEMENT_LISTENER();
