...
//#ifndef _DEBUG
//void ZGameClient::RegisterFunctions()
//{
//	//m_pLua->Bind("chatOutput", &this->OnStageChat);
//}
//#endif
...
//	MMatchObjectCache* pObjCache = FindObjCache(uid);
	MCOLOR _color = MCOLOR(205,205,205);
...
void ZGameClient::OnStageLeave(const MUID& uidChar, const MUID& uidStage)
{
	...
	if (uidChar == GetPlayerUID())
	{
		...
		for (int i = 0; i < ZGetMyInfo()->GetStageModItems().size(); ++i)
		{
			std::pair<int, MMatchCharItemParts> itemPair = ZGetMyInfo()->GetStageModItems().at(i);
			MMatchItemDesc* item = MGetMatchItemDescMgr()->GetItemDesc(itemPair.first);
			if (item != nullptr)
			{
				ZMyItemNode* uidItem = ZGetMyInfo()->GetItemList()->GetItemByDescId(item->m_nID);
				if (uidItem == nullptr)
				{
					//Something weird happened???
					continue;
				}
				MMatchCharItemParts parts = itemPair.second;
				if (parts == MMCIP_END)
					continue;
				ZPostRequestEquipItem(uidChar, uidItem->GetUID(), parts);
			}
		}
		ZGetMyInfo()->RemoveStageModItems();
	}
...
void ZGameClient::OnStageList(int nPrevStageCount, int nNextStageCount, void* pBlob, int nCount)
{
	...
	for(int i=0; i<nCount; i++) {
		...
		if( pNode ) 
		{
			...
			{
				roominfo.stageMods[i] = pNode->stageMods[i];
			}
...
void ZGameClient::ChangeQuestStage()
{
	...
	if( (0 != ZGetGameClient()) && 
		...
		(0 == _stricmp(MCHANNEL_RULE_QUEST_STR, ZGetGameClient()->GetChannelRuleName())) &&
...
void ZGameClient::CheckStageJoinMods(const MUID& player)
{
	if (ZGetGameClient()->GetMatchStageSetting()->GetStageModCount() > 0)
	{
		///Items have already been stored, prevent multiple storing.
		if (ZGetMyInfo()->GetStageModItems().size() > 0)
			return;
		if (ZGetGameClient()->GetMatchStageSetting()->isModEnabled(SM_NS))
		{
			ZMyItemNode* item = ZGetMyInfo()->GetItemList()->GetEquipedItem(MMCIP_MELEE);
			if (item != nullptr)
			{
				ZPostRequestTakeoffItem(player, MMCIP_MELEE);
				ZGetMyInfo()->AddStageModRemovedItem(item->GetItemID(), MMCIP_MELEE);
			}
		}
		if (ZGetGameClient()->GetMatchStageSetting()->isModEnabled(SM_NA)) {
			for (int i = MMCIP_HEAD; i < MMCIP_FINGERL; ++i) {
				ZMyItemNode* item = ZGetMyInfo()->GetItemList()->GetEquipedItem((MMatchCharItemParts)i);
				if (item != nullptr)
				{
					ZPostRequestTakeoffItem(player, (MMatchCharItemParts)i);
					ZGetMyInfo()->AddStageModRemovedItem(item->GetItemID(), (MMatchCharItemParts)i);
				}
			}
		}
		if (ZGetGameClient()->GetMatchStageSetting()->isModEnabled(SM_NG)) {
			for (int i = MMCIP_PRIMARY; i < MMCIP_CUSTOM1; ++i) {
				ZMyItemNode* item = ZGetMyInfo()->GetItemList()->GetEquipedItem((MMatchCharItemParts)i);
				if (item != nullptr)
				{
					ZPostRequestTakeoffItem(player, (MMatchCharItemParts)i);
		...
		if (ZGetGameClient()->GetMatchStageSetting()->isModEnabled(SM_NI)) {
			for (int i = MMCIP_CUSTOM1; i <= MMCIP_AVATAR; ++i) {
				ZMyItemNode* item = ZGetMyInfo()->GetItemList()->GetEquipedItem((MMatchCharItemParts)i);
				if (item != nullptr)
				{
					ZPostRequestTakeoffItem(player, (MMatchCharItemParts)i);
		...
		if (ZGetGameClient()->GetMatchStageSetting()->isModEnabled(SM_NR)) {
			for (int i = MMCIP_FINGERL; i < MMCIP_MELEE; ++i) {
				ZMyItemNode* item = ZGetMyInfo()->GetItemList()->GetEquipedItem((MMatchCharItemParts)i);
				if (item != nullptr)
				{
					ZPostRequestTakeoffItem(player, (MMatchCharItemParts)i);
