...
void DrawInfectionGauge(float x, float y, float x2, float y2, DWORD color)
{
	for (int i = 0; i<40; i++)
	{
		float fAngle = i * 2 * pi / (float)40;
		float fAngle2 = (i + 1) * 2 * pi / (float)40;

		rvector a = 50*rvector(cos(fAngle), sin(fAngle), 0) + rvector(x,y,0);
		a.x -= x2;
		a.y -= y2;
		rvector b = 50*rvector(cos(fAngle2), sin(fAngle2), 0) + rvector(x,y,0);
		b.x -= x2;
		b.y -= y2;

		RDrawCircle2(x, y, a, b, 50,i, color);
	}
}
...
void ZScreenEffectManager::ReSetHpPanel()
{
	...
	//{
	//	if (m_pInfectionPanel->GetVMesh())
	//	{
	//		AniFrameInfo* pAni = m_pInfectionPanel->GetVMesh()->GetFrameInfo(ani_mode_lower);
	...
	//		m_pInfectionPanel->GetVMesh()->SetAnimation("play");
	//	}
	//}
	//else
	{
...
void ZScreenEffectManager::DrawInfectionGauges()
{
	RGetDevice()->SetFVF(D3DFVF_XYZRHW | D3DFVF_TEX1 | D3DFVF_DIFFUSE);
	...
	DrawInfectionGauge(70.f / 800.f, 23.f / 600.f, min(1.f, m_fGaugeHP) * 138.f / 800.f, 13.f / 600.f, color);

	// ap

	color = D3DCOLOR_ARGB(255, 68, 193, 62);

	DrawInfectionGauge(84.f / 800.f, 50.f / 600.f, min(1.f, m_fGaugeAP) * 138.f / 800.f, 13.f / 600.f,color);
	...
	if (render_cur_hp) {
		...
		DrawInfectionGauge(70.f / 800.f + x, 23.f / 600.f, w, 13.f / 600.f, color);
	}
	...
	if (render_cur_ap) {
		...
		DrawInfectionGauge(84.f / 800.f + x, 50.f / 600.f, w, 13.f / 600.f, color);
	}
...
void ZScreenEffectManager::DrawGauges()
{
	...
	DrawGauge(70.f/adjustWidth , 23.f/600.f , min(1.f,m_fGaugeHP) * 138.f/adjustWidth , 13.f/600.f , 1.f ,color);

	// ap

	color = D3DCOLOR_ARGB(255, 68,193, 62);

	DrawGauge(84.f/adjustWidth , 50.f/600.f , min(1.f,m_fGaugeAP) * 138.f/adjustWidth , 13.f/600.f , -1.f ,color);
	...
	if(render_cur_ap) {
		...
		float x = min(1.f,m_fGaugeAP) * 138.f/adjustWidth;	
		float w = (m_fCurGaugeAP-m_fGaugeAP) * 138.f/adjustWidth;

		DrawGauge(84.f/adjustWidth+x , 50.f/600.f , w , 13.f/600.f , -1.f ,color);
	}
...
void ZCombatInterface::DrawHPAPNumbers(MDrawContext* pDC)
{
	...
	if (ZGetConfiguration()->GetVideo()->bHPAP) {
		...
		sprintf(hp, "HP : %d", static_cast<int>(pCharacter->GetHP())); //(int)pCharacter->GetHP()
	...
	if (ZGetConfiguration()->GetVideo()->bHPAP) {
		...
		sprintf(ap, "AP : %d", static_cast<int>(pCharacter->GetAP())); //(int)pCharacter->GetAP())
...
void ZScreenEffectManager::DrawInfectionPanel(MDrawContext* pDC)
{
	if (ZGetGameInterface()->GetCombatInterface()->GetObserverMode()) return;
	...
	if (ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_INFECTION) return;
	if (!pTargetCharacter->IsObserverTarget())
	{
		m_pInfectionPanel->Update();

		m_pInfectionPanel->Draw(0);

		bool bDrawGauges = false;
		// 에니메이션이 끝난후에~
		//if (m_pInfectionPanel->GetVMesh())
		//	if (m_pInfectionPanel->GetVMesh()->GetFrameInfo(ani_mode_lower)->m_isPlayDone)
				bDrawGauges = true;
...
//Custom: gametype roundresult
void ZScreenEffectManager::AddRoundResult(MMATCH_GAMETYPE gameType, MMATCH_ROUNDRESULT roundResult)
{
	switch (gameType)
	{
		case MMATCH_GAMETYPE_INFECTION:
		{
			if (roundResult == MMATCH_ROUNDRESULT_REDWON || roundResult == MMATCH_ROUNDRESULT_RED_ALL_OUT)
			{
				AddInfectedWin();
			}
			else if (roundResult == MMATCH_ROUNDRESULT_BLUEWON || roundResult == MMATCH_ROUNDRESULT_BLUE_ALL_OUT)
			{
				AddSurvivorsWin();
			}
		}
...
void ZScreenEffectManager::SetOffSet()
{
	return;
	//TODO: update offset for each screeneffect that exists
	rvector offSet(0, 0, 0), offSet2(0, 0, 0);
	if (RGetIsWidthScreen())
	{
		offSet = rvector(-90, 0, 0);
		offSet2 = rvector(90, 0, 0);
	}
	m_pHPPanel->SetOffset(offSet);
	m_pScorePanel->SetOffset(offSet2);
	m_pBuffPanel->SetOffset(offSet);

	for (int i = 1; i < MWT_END; ++i)
	{
		if (i >= 25 && i <= 31)
			continue;
		m_pWeaponIcons[i]->SetOffset(offSet2);
	}
}