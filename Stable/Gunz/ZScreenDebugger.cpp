...
#define NEXTLINE() { sy+=15; }
	...
	if(m_nShowDebugInfo==2) {
		...
		sprintf(buffer, "FPS : %3.3f , ms %4.1f,score %4.1f 점    Send:%d CPS, Recv:%d CPS    CmdQueue: %d RenderCharCnt = %d/%d", g_fFPS, fMs, fScore, nSendTraffic, nRecvTraffic,
	...
	sprintf(buffer, "FPS : %3.3f , %d polygon , %d calls , %4.1f ms,score %4.1f 점, CmdQueue: %d",g_fFPS,g_nPoly,g_nCall,fMs,fScore,
	...
	if(pVMesh) {
		...
		if( pAniInfo[0]->m_pAniSet ) {
			...
			for(int i=0;i<2;i++) {
				...
				sprintf(buffer,"speed %d = %d 퍼센트 ( %d/%d ) ms (delay = %d) ",i, speed, org_time , delay_time,delay_time-org_time);
				pDC->SetColor(MCOLOR(0xFFffffff));
				OUTTEXT();
				NEXTLINE();
...
//	ZGetGame()->GetWorld()->GetBsp()->DrawNavi_Links();
	...
	// 충돌체크
	{
			...
			itor != ZGetGame()->m_ObjectManager.end(); ++itor)
		{
			...
			if(pObj->IsNPC() && MIsExactlyClass(ZActor, pObj))
			{
	...
	for (ZObjectManager::iterator itor = pcm->begin(); itor != pcm->end(); ++itor)
	{
		...
		if (pObject->IsNPC() && MIsExactlyClass(ZActor, pObject))
		{
...
//#ifndef _PUBLISH
	switch (m_nShowDebugInfo)
	{
		...
		case 1: m_nShowDebugInfo = 0; break;
	}
//#endif
}