...
//	RMesh*	m_pRocketSmokeEffect;
	...
	// hp ap 회복 이펙트
	RMesh*	m_pHealEffect;				// 월드아이템 섭취
	RMesh*	m_pRepairEffect;
	RMesh*	m_pHealInstantEffect;		// 포션류로 인한 순간 회복
	RMesh*	m_pRepairInstantEffect;
	RMesh*	m_pHealOverTimeEffect;		// 포션류로 인한 지속 회복
	RMesh*	m_pRepairOverTimeEffect;
	RMesh*	m_pHasteEffect;
	RMesh*	m_pHealOverTimeBeginEffect;		// 지속 회복의 시작 이펙트
...
#define BILLBOARDTEXANILIST_COUNT 5
	...
	bool GetEffect(ZEffect* currEffect);
