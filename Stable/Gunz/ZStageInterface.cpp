...
void ZStageInterface::OnCreate( void)
{
	...
	//Custom: spymode
	{
		///TODO: change this to spylist image -> the create function call
		pPicture = (MPicture*)pResource->FindWidget("Stage_SpyBanMapListBG");
		if (pPicture)
		{ // f릴레이맵 리스트 배경 이미지
			m_spyMapList = new MBitmapR2;
			((MBitmapR2*)m_spyMapList)->Create("relaymaplistframe.tga", RGetDevice(), "interface/loadable/relaymaplistframe.tga");

			if (m_pRelayMapListFrameImg != NULL)
				pPicture->SetBitmap(m_spyMapList->GetSourceBitmap());
		}
		...
		pLabel = (MLabel*)pResource->FindWidget("Stage_SpyBanMapLabel");
		if (pLabel)
			pLabel->SetText("");
...
#define SURVIVAL_COLOR		MCOLOR(255,255,255)
	...
	{	
		...
		case MMATCH_GAMETYPE_QUEST_CHALLENGE:
			color = QUEST_COLOR; //todok 퀘스트챌린지 색깔?
			break;
...
void ZStageInterface::ChangeStageGameSetting( const MSTAGE_SETTING_NODE* pSetting)
{
	...
	/*	(pSetting->nGameType == MMATCH_GAMETYPE_CLASSIC_SOLO) ||*/
		...
		(pSetting->nGameType == MMATCH_GAMETYPE_INFECTION))							// 듀얼모드 이면...
	{
		// 맵 이름 배경 이미지 변환
		if (pAniMapImg)
	...
	else if (pSetting->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE)						// 퀘스트 모드이면...
	{
		// 맵 이름 배경 이미지 변환
		if (pAniMapImg)
...
//		ZApplication::GetGameInterface()->EnableWidget( "StageMaxPlayer", false);
	...
	if (m_nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE)
	{
		ZApplication::GetGameInterface()->EnableWidget("StageMaxPlayer", false);
	}
...
void ZStageInterface::CreateSpyBanMapList()
{
	...
	int playerCount = ZGetGameClient()->GetMatchStageSetting()->m_CharSettingList.size();

	for (int i = 0; i < MGetSpyMode()->GetSpyMapList().size(); ++i)
	{
		char mapInfo[256] = "";
		auto map = MGetSpyMode()->GetSpyMapList().at(i);
		sprintf_s(mapInfo, "%s (%d~%d Players)", map.name, map.minPlayers, 
			map.maxPlayers);
		bool canPlayMap = !(playerCount < map.minPlayers || playerCount > map.maxPlayers);
		SpyMapList* pMap = new SpyMapList(mapInfo,
			canPlayMap ? MBitmapManager::Get("Mark_Arrow.bmp") :
			MBitmapManager::Get("Mark_X.bmp"),canPlayMap ? MCOLOR(DEFCOLOR_MLIST_TEXT) : MCOLOR(160,0,0));
		pListBox->Add(pMap);
	}
}
...
bool ZStageInterface::OnNewQuestGameInfo(int const& maxLevel, int const& levelLimit)
{
	return false;//todo;
}
...
bool ZStageInterface::ReadSenarioNameXML(void)
{
	...
	{
		mlog("Error while read : scenario.xml\n");
		return false;
	}
	...
	if (!doc.parse<0>(buffer)) {
		mlog("Error parsing scenario.xml\n");
		delete[] buffer;
		mzFile.Close();
		return false;
	}
	...
	for (auto itor = rootNode->first_node(); itor; itor = itor->next_sibling()) {
		...
		if (_stricmp(itor->name(), "STANDARD_SCENARIO") == 0)
		{
			SenarioMapList.m_ScenarioType = ST_STANDARD;
			strcpy_s(SenarioMapList.m_szName, itor->first_attribute("title")->value());
			strcpy_s(SenarioMapList.m_szMapSet, itor->first_attribute("mapset")->value());

			nItemID = nStdScenarioCount++;					// ID

			bFind = true;
		}
		if (_stricmp(itor->name(), "SPECIAL_SCENARIO") == 0)
		{

			SenarioMapList.m_ScenarioType = ST_SPECIAL;
			nItemID = atoi(itor->first_attribute("id")->value());
			strcpy_s(SenarioMapList.m_szName, itor->first_attribute("title")->value());
			strcpy_s(SenarioMapList.m_szMapSet, itor->first_attribute("mapset")->value());
