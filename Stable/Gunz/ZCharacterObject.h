...
enum ZC_STATE_LOWER {
	...
	ZC_STATE_PIT,
	///TODO: add new animation state
	ZC_STATE_PLANTING_BOMB,
	//ZC_STATE_BOMB_PLANTED,

	ZC_STATE_LOWER_END
};

// ZCharacter 와 ZActor A collection of writing things like class ..

class ZShadow;


// Send and receive data between a certain time intervals to keep the character
struct ZBasicInfo {
	rvector position;
	rvector velocity;
//	rvector accel;
...
public:
	...
	void DrawShadow();		// ManagerShall be called by the.
...
public:
	...
	bool	m_bLeftShot;				// If the total of both hands, alternating between flashing ...Flashed ...
	...
	bool		m_bDynamicLight;		// Additional light effect?
